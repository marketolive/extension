...
function getMktoAppParms() {
	if (typeof(MktPage) !== "undefined") {
		...
		current_pod_string = MktPage.savedState.mainAppBaseUrl;
	}else{
		return;
	}
	function getCustPrefix(cust_prefix) {
		console.log(cust_prefix);
		//setCookie('custPrefix', cust_prefix, 365, 'marketo.com', true);
		if (cust_prefix !== "undefined" || cust_prefix !== null) {
			window.postMessage({
				type : 'custPrefix',
				text : cust_prefix
			}, mkto_app_url);
	...
	function getCurrentPod(current_pod_string) {
		current_pod_string_split = current_pod_string.split('//');
		current_pod_values = current_pod_string_split[1].split('.');
		current_pod = current_pod_values[0]; 
		console.log(current_pod);
		//setCookie('userPod', current_pod, 365, 'marketo.com', true);
		if (current_pod !== "undefined" || current_pod !== null) {
			window.postMessage({
				type : 'userPod',
				text : current_pod
			}, mkto_app_url);
	...
	var custPrefixStatus = getCustPrefix(cust_prefix);
	console.log(custPrefixStatus);
	var currentPodStatus = getCurrentPod(current_pod_string);
