...
if(accountString.search(mktoAccountStringsMatch)!=-1||accountString==mktoAccountStringQe||APP.getCookie("toggleState")=="false"){console.log("Marketo App > Location: MarketoLive Instance");window.mkto_live_plugin_state=true;if(userId.search("^admin\.?[a-z]{0,2}@mktodemoaccount")!=-1||userId.search("^mktodemoaccount[a-z0-9]*@marketo\.com")!=-1||userId.search("^marketodemo.*@gmail\.com$")!=-1){console.log("Marketo App > User: Admin");APP.disableDemoPluginCheck();if(APP.getCookie("toggleState")!="false"){return;}
...
window.onhashchange=function(){currUrlFragment=Mkt3.DL.getDlToken();if(currUrlFragment==mktoMyMarketoFragment){APP.overrideHomeTiles();}}}}},0);}