...
if(item){if(itemToHide.action=="setVisible"){item.setVisible(false);}else if(itemToHide.action=="setDisabled"){item.setDisabled(true);}}});}},0);};APP.disableModelerSaving=function(mode){console.log("Marketo App > Disabling: Revenue Cycle Model (Edit/Preview) Saving");var isRevenueCycleModel;isRevenueCycleModel=window.setInterval(function(){if(typeof(MktCanvas)!=="undefined"&&MktCanvas&&MktCanvas.getActiveTab()&&MktCanvas.getActiveTab().config&&MktCanvas.getActiveTab().config.accessZoneId){window.clearInterval(isRevenueCycleModel);var assetNode=MktCanvas.getActiveTab().config,heapEvent={name:"",assetName:assetNode.satelliteTitle,assetType:assetNode.compType,assetId:assetNode.expNodeId,workspaceId:assetNode.accessZoneId,workspaceName:""},titleReplaceRegex=new RegExp("\\([^\\)]+\\)$");switch(mode){case"edit":APP.disableSaving();heapEvent.assetArea="Editor";break;case"preview":heapEvent.assetArea="Previewer";break;}
if(assetNode.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){heapEvent.workspaceName=APP.getWorkspaceName(assetNode.accessZoneId);heapEvent.name=heapEvent.workspaceName+" > "+heapEvent.assetType;}else if(assetNode.accessZoneId==mktoUserWorkspaceId){heapEvent.name=userWorkspaceName+" > "+userName;heapEvent.workspaceName=userWorkspaceName;}else{heapEvent.name="User's Workspace > "+userName;heapEvent.workspaceName="User's Workspace";}
if(heapEvent.assetName.search(titleReplaceRegex)!=-1){heapEvent.assetName=heapEvent.assetName.replace(titleReplaceRegex).trimRight();}
if(heapEvent.assetName.search(/"/)!=-1){heapEvent.assetName=heapEvent.assetName.replace(/"/g,"");}
heapTrack("track",heapEvent);}},0);};APP.disableDesignerSaving=function(assetType,mode){console.log("Marketo App > Disabling: Designer (Edit/Preview) Saving & Toolbar Menus for "+assetType);var isAppController;isAppController=window.setInterval(function(){if(typeof(Mkt3)!=="undefined"&&Mkt3&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get){window.clearInterval(isAppController);var disableDesignerAsset,assetNode,menuItems;disableDesignerAsset=function(assetNode,menuItems,disableFunc){console.log("Marketo App > Executing: Disabling Designer (Edit/Preview)");var heapEvent={name:"",assetName:"",assetType:assetNode.compType,assetId:assetNode.id,workspaceId:assetNode.accessZoneId,workspaceName:""};switch(mode){case"edit":heapEvent.assetArea="Editor";break;case"preview":heapEvent.assetArea="Previewer";break;default:heapEvent.assetArea="Designer";break;}
if(assetNode.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){heapEvent.workspaceName=APP.getWorkspaceName(assetNode.accessZoneId);heapEvent.name=heapEvent.workspaceName+" > "+heapEvent.assetType;}else if(assetNode.accessZoneId==mktoUserWorkspaceId){heapEvent.name=userWorkspaceName+" > "+userName;heapEvent.workspaceName=userWorkspaceName;}else{heapEvent.name="User's Workspace > "+userName;heapEvent.workspaceName="User's Workspace";}
...
APP.trackNodeClick=function(){console.log("Marketo App > Tracking: Tree Node Click");if(typeof(Ext)!=="undefined"&&Ext&&Ext.tree&&Ext.tree.TreeEventModel&&Ext.tree.TreeEventModel.prototype&&Ext.tree.TreeEventModel.prototype.onNodeClick){Ext.tree.TreeEventModel.prototype.onNodeClick=function(e,node){if(node&&node.text&&node.attributes&&node.attributes.accessZoneId){var currNode=node,heapEventName,heapEvent={name:"",assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,assetPath:"",workspaceId:currNode.attributes.accessZoneId,workspaceName:""};if(currNode.attributes.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){heapEvent.assetPath=currNode.text;for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEvent.assetPath=currNode.text+" > "+heapEvent.assetPath;}
heapEvent.workspaceName=currNode.text;if(heapEvent.workspaceName!="Admin"){heapEventName=heapEvent.workspaceName+" > "+heapEvent.assetType;}else{heapEventName=heapEvent.assetPath;heapEvent.assetId=0;}}else if(currNode.attributes.accessZoneId==mktoUserWorkspaceId){heapEventName=userWorkspaceName+" > "+userName;heapEvent.workspaceName=userWorkspaceName;}else{for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;}
heapEventName=heapEvent.workspaceName=currNode.text;}
heapEvent.name=heapEventName;heapTrack("track",heapEvent);}
node.ui.onClick(e);}}};var heapTrack=function(action,event){var isHeapAnalytics=window.setInterval(function(){if(typeof(heap)!=="undefined"&&heap){window.clearInterval(isHeapAnalytics);switch(action){case"id":var oneLoginEmail=APP.getCookie("onelogin_email"),oneLoginFirstName=APP.getCookie("onelogin_first_name"),oneLoginLastName=APP.getCookie("onelogin_last_name");if(oneLoginEmail){heap.identify(oneLoginEmail);}else if(MktPage&&MktPage.userid){heap.identify(MktPage.userid);}
