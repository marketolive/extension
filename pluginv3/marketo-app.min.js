...
Ext.menu.Menu.prototype.showAt=function(xy,parentMenu){console.log("Marketo App > Executing: Disabling Right-click and Actions menus for ALL in Analytics");if(this.fireEvent('beforeshow',this)!==false){var mItems=this.items,canvas=MktCanvas.getActiveTab(),disable=APP.evaluateMenu(this.triggeredFrom,attr,this,canvas,null),itemsToDisable=["cloneReport","deleteReport","moveReport","rcmApproveStages","rcmUnapproveStages","rcmApprove","rcmUnapprove","rcmClone","rcmDelete","rcmExport","rcmApproveDraft","rcmAassignmentRules",];itemsToDisable.forEach(function(itemToDisable){var item=mItems.get(itemToDisable);if(item){item.setDisabled(disable);}});this.parentMenu=parentMenu;if(!this.el){this.render();}
