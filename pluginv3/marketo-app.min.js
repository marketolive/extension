...
APP.trackNodeClick=function(){console.log("Marketo App > Tracking: Tree Node Click");if(typeof(Ext)!=="undefined"&&Ext&&Ext.tree&&Ext.tree.TreeEventModel&&Ext.tree.TreeEventModel.prototype&&Ext.tree.TreeEventModel.prototype.onNodeClick){Ext.tree.TreeEventModel.prototype.onNodeClick=function(e,node){if(node&&node.text&&node.attributes&&node.attributes.accessZoneId){var currNode=node,heapEventName,heapEvent={assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,workspaceId:currNode.attributes.accessZoneId};if(currNode.attributes.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){heapEventName=currNode.text;for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" > "+heapEventName;}}else if(currNode.attributes.accessZoneId==mktoUserWorkspaceId){heapEventName=userWorkspaceName+" > "+userName;}else{for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;}
