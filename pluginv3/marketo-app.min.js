...
if(currUrlFragment.search("^"+mktoEmailDesignerFragment)==-1&&currUrlFragment.search("^"+mktoEmailPreviewFragment)&&currUrlFragment.search("^"+mktoLandingPageDesignerFragment)==-1&&currUrlFragment.search("^"+mktoLandingPagePreviewFragment)==-1&&currUrlFragment.search("^"+mktoFormWizardFragment)==-1&&currUrlFragment.search("^"+mktoMobilePushNotificationWizardFragment)==-1&&currUrlFragment.search("^"+mktoSocialAppWizardFragment)==-1){APP.overrideTreeNodeExpand();APP.overrideTreeNodeCollapse();APP.overrideNewProgramCreate();APP.overrideProgramSaveEdit();APP.overrideNewSmartCampaignCreate();APP.hidePageGrid();APP.hideFoldersOnImport();APP.disableConfirmationMessage();if(currUrlFragment!=mktoMyMarketoFragment){var isMktCanvas=window.setInterval(function(){if(MktCanvas.activeTab!==null){console.log("Marketo App > Location: Marketo Canvas");window.clearInterval(isMktCanvas);prevWorkspaceId=MktCanvas.activeTab.config.accessZoneId;if(prevWorkspaceId==1||prevWorkspaceId==japanWorkspaceId){APP.disableSmartCampaignSaving();APP.enableSmartCampaignCanvas();APP.disableSaving();}}},0);}
...
else if(currWorkspaceId==1||currWorkspaceId==japanWorkspaceId){APP.disableSmartCampaignSaving();APP.enableSmartCampaignCanvas();APP.disableSaving();prevWorkspaceId=currWorkspaceId;}
