...
return false;}
isEmailEditor2=window.setInterval(function(){var pointerCursorEls=document.getElementsByTagName("iframe")[0].contentWindow.document.getElementsByTagName("body")[0].getElementsByClassName("pointerCursor");prevLastPointerCursorEl=currLastPointerCursorEl;currLastPointerCursorEl=pointerCursorEls[pointerCursorEls.length-1];if(currLastPointerCursorEl!=prevLastPointerCursorEl){if(action=="edit"){console.log("Marketo App > Overlaying: Email Editor");if(overlay(document.getElementsByTagName("iframe")[0].contentWindow.document)){console.log("Marketo App > Overlaying: Email Interval is Cleared");window.clearInterval(isEmailEditor2);}}
else if(action=="preview"){console.log("Marketo App > Overlaying: Email Previewer");if(document.getElementsByTagName("iframe")[2].contentWindow.document){if(count>5000||overlay(document.getElementsByTagName("iframe")[2].contentWindow.document)){window.clearInterval(isEmailEditor2);}}}}
else{console.log("Marketo App > Overlaying: Email Interval is Cleared by Force");window.clearInterval(isEmailEditor2);}},0);}
...
window.onhashchange=function(){console.log("Marketo App > Window: Hash Changed");currentUrl=window.location.href;var isNewUrlFragment=window.setInterval(function(){if(Mkt3&&Mkt3.DL&&Mkt3.DL.getDlToken()&&Mkt3.DL.dl&&Mkt3.DL.dl.dlCompCode){if(currUrlFragment!=Mkt3.DL.getDlToken()){console.log("Marketo App > Loaded: New URL Fragment");window.clearInterval(isNewUrlFragment);currUrlFragment=Mkt3.DL.getDlToken();currCompFragment=Mkt3.DL.dl.dlCompCode;console.log("Marketo App > Window: URL Fragment = "+currUrlFragment);console.log("Marketo App > Window: Comp Fragment = "+currCompFragment);if(currUrlFragment==mktoMyMarketoFragment){APP.overrideHomeTiles();}
else if(currUrlFragment==mktoMarketingActivitiesDefaultFragment||currUrlFragment==mktoMarketingActivitiesMarketingFragment||currUrlFragment==mktoMarketingActivitiesJapaneseFragment||currUrlFragment==mktoMarketingActivitiesFinservFragment||currUrlFragment==mktoMarketingActivitiesHealthcareFragment||currUrlFragment==mktoMarketingActivitiesHigherEdFragment||currUrlFragment==mktoLeadDatabaseDefaultFragment||currUrlFragment==mktoLeadDatabaseMarketingFragment||currUrlFragment==mktoLeadDatabaseJapaneseFragment||currUrlFragment==mktoLeadDatabaseFinservFragment||currUrlFragment==mktoLeadDatabaseHealthcareFragment||currUrlFragment==mktoLeadDatabaseHigherEdFragment){APP.disableButtons();}
else if(currUrlFragment==mktoAnalyticsDefaultFragment){APP.overrideAnalyticsTiles();}
