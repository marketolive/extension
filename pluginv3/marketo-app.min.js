...
APP.trackTreeNodeSelection=function(){console.log("Marketo App > Tracking: Tree Node Selection");if(typeof(MktExplorer)!=="undefined"&&MktExplorer.selectTreeNode){MktExplorer.selectTreeNode=function(node){var currNode=node,heapEventName,heapEvent={};if(currNode.attributes.accessZoneId!=mktoUserWorkspaceId){heapEventName=currNode.text;heapEvent={assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,workspaceId:currNode.attributes.accessZoneId};for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" | "+heapEventName;}}else{heapEventName=userWorkspaceName+" | "+userName;}
heapEvent.name=heapEventName;heapTrack("track",heapEvent);if(!node||!this.boundTree)
return false;if(!node.isSelected()){this.boundTree.expandPath(node.getPath(),'id');node.render();node.select();return true;}else{return false;}};}};APP.trackOtherAssets=function(){console.log("Marketo App > Tracking: Social App or Push Notification Selection");if(typeof(MktExplorer)!=="undefined"&&MktExplorer.getNodeById&&MktExplorer.getNodeById(currUrlFragment.substring(0,currUrlFragment.length-5))){var node=currNode=MktExplorer.getNodeById(currUrlFragment.substring(0,currUrlFragment.length-5)),heapEventName,heapEvent={};if(currNode.attributes.accessZoneId!=mktoUserWorkspaceId){heapEventName=currNode.text;heapEvent={assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,workspaceId:currNode.attributes.accessZoneId};for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" | "+heapEventName;}}else{heapEventName=userWorkspaceName+" | "+userName;}
