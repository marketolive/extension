...
overlay=function(emailDocument){if(emailDocument!=null){console.log(emailDocument);var emailBody=emailDocument.getElementsByTagName("body")[0],logoSwapCompany=emailDocument.getElementById("logo-swap-company"),logoSwapContainer=emailDocument.getElementById("logo-swap-container"),logoSwapCompanyContainer=emailDocument.getElementById("logo-swap-company-container"),logoBkg=emailDocument.getElementById("logo-bkg"),buttonBkg=emailDocument.getElementById("button-bkg");if(emailBody!=null){console.log(emailBody);var mktoImgs=emailBody.getElementsByClassName("mktoImg"),mktoTexts=emailBody.getElementsByClassName("mktoText"),mktoButtons=emailBody.getElementsByClassName("secondary-font button");if(!isMktoImgReplaced&&mktoImgs.length!=0){console.log(mktoImgs);var ii,currMktoImg,currMktoImgMktoName,currMktoImgTag;for(ii=0;ii<mktoImgs.length;ii++){currMktoImg=mktoImgs[ii];currMktoImgMktoName=currMktoImg.getAttribute("mktoname");if(currMktoImgMktoName&&currMktoImgMktoName.search(logoMktoNameRegex)!=-1){currMktoImgTag=currMktoImg.getElementsByTagName("img")[0];if(currMktoImgTag&&currMktoImgTag.src){console.log("Marketo App > Overlaying: Email 2.0 Company Logo");currMktoImgTag.setAttribute("src",logo);isMktoImgReplaced=true;break;}}}}
...
return false;}
isEmailEditor2=window.setInterval(function(){if(action=="edit"){console.log("Marketo App > Overlaying: Email Editor");if(overlay(document.getElementsByTagName("iframe")[0].contentWindow.document)){console.log("Marketo App > Overlaying: Email Interval is Cleared");window.clearInterval(isEmailEditor2);}}
else if(action=="preview"){console.log("Marketo App > Overlaying: Email Previewer");if(!isDesktopPreviewReplaced&&overlay(document.getElementsByTagName("iframe")[2].contentWindow.document)){isDesktopPreviewReplaced=true;}
if(!isPhonePreviewReplaced&&overlay(document.getElementsByTagName("iframe")[3].contentWindow.document)){isPhonePreviewReplaced=true;}
