...
var ii,currNode=node,heapEventName=currNode.text,heapEvent={assetName:currNode.text};for(ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" | "+heapEventName;}
...
else{return false;}};}};APP.trackOtherAssets=function(){console.log("Marketo App > Tracking: Social App or Push Notification Selection");if(typeof(MktExplorer)!=="undefined"&&MktExplorer.getNodeById&&MktExplorer.getNodeById(currUrlFragment.substring(0,currUrlFragment.length-5))){var ii,node=currNode=MktExplorer.getNodeById(currUrlFragment.substring(0,currUrlFragment.length-5)),heapEventName=currNode.text,heapEvent={assetName:currNode.text};for(ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" | "+heapEventName;}
heapEvent.name=heapEventName;heapTrack("track",heapEvent);}};var heapTrack=function(action){var isHeapAnalytics=window.setInterval(function(){if(typeof(heap)!=="undefined"){console.log("Marketo App > Loaded: Heap Analytics");window.clearInterval(isHeapAnalytics);switch(action,event){case"id":var oneLoginEmail=APP.getCookie("onelogin_email"),oneLoginFirstName=APP.getCookie("onelogin_first_name"),oneLoginLastName=APP.getCookie("onelogin_last_name");if(oneLoginEmail){heap.identify(oneLoginEmail);}
...
if(MktPage&&MktPage.baseTitle){heapArea=MktPage.baseTitle.split("•")[0].trimRight();}
...
else if(currUrlFragment.search(mktoOtherAssetsFragmentMatch)){APP.trackOtherAssets();}
