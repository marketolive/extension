...
var params={compId:this.programId,portletOrdering:Ext.encode(newPortletOrder)};MktSession.ajaxRequest('marketingEvent/orderLocalAssetPortlets',{serializeParms:params,localAssetManager:this,portletOrdering:newPortletOrder,onMySuccess:this.updatePortletOrderSuccess});}else{console.log("Marketo App > Disabling: Updating of Portlet Order");}};}};APP.overrideTreeNodeExpand=function(){console.log("Marketo App > Overriding: Tree Node Expand");if(typeof(MktAsyncTreeNode)!=="undefined"&&MktAsyncTreeNode&&MktAsyncTreeNode.prototype&&MktAsyncTreeNode.prototype.expand&&userName){MktAsyncTreeNode.prototype.expand=function(){console.log("Marketo App > Executing: Tree Node Expand");var attr=this.attributes,ii;if(this.text==userWorkspaceName||(this.parentNode.text==userWorkspaceName&&this.attributes.system==true)||(this.parentNode.parentNode!=null&&this.parentNode.parentNode.text==userWorkspaceName&&this.attributes.system==true)){for(ii=0;ii<this.childNodes.length;ii++){if(this.childNodes[ii].attributes.system==false){if(this.childNodes[ii].text.toLowerCase()!==userName){this.childNodes[ii].hidden=true;}else{}}}}
...
MktAsyncTreeNode.superclass.expand.apply(this,arguments);attr.mktExpanded=true;};}};APP.overrideTreeNodeCollapse=function(){console.log("Marketo App > Overriding: Tree Node Collapse");if(typeof(MktAsyncTreeNode)!=="undefined"&&MktAsyncTreeNode&&MktAsyncTreeNode.prototype&&MktAsyncTreeNode.prototype.collapse&&userName){MktAsyncTreeNode.prototype.collapse=function(){console.log("Marketo App > Executing: Tree Node Collapse");var attr=this.attributes,ii;if(this.text==userWorkspaceName||(this.parentNode.text==userWorkspaceName&&this.attributes.system==true)||(this.parentNode.parentNode!=null&&this.parentNode.parentNode.text==userWorkspaceName&&this.attributes.system==true)){for(ii=0;ii<this.childNodes.length;ii++){if(this.childNodes[ii].attributes.system==false){if(this.childNodes[ii].text.toLowerCase()!==userName){this.childNodes[ii].ui.elNode.hidden=true;}else{}}}}
...
var disable=APP.evaluateMenu("inAppMessage",null,null,this),menuItems=["inAppMessage contextMenu [action=unapprove]","inAppMessage contextMenu [action=approve]","inAppMessage contextMenu [action=clone]","inAppMessage contextMenu [action=delete]","inAppMessage contextMenu [action=sendDraftSample]","inAppMessage contextMenu [action=approveDraft]",],mItems=Ext4.ComponentQuery.query(menuItems.toString());if(mItems){mItems.forEach(function(item){if(item){item.setDisabled(disable);}});}};}else{console.log("Marketo App > Skipped: Disable Toolbar Buttons & Actions Menu for Marketing Activities > In-App Messages");}
...
{var menuItems=["[action=submit]",],mItems=this.query(menuItems.toString());if(mItems){mItems.forEach(function(item){if(item){item.setDisabled(true);}});}}
...
currPosition=x;}
window.location=pod.valueSet[currPosition].url;});}}}},0);};APP.overrideSaving=function(){console.log("Marketo App > Overriding: Saving for Nurture Streams");if(typeof(Mkt3)!=="undefined"&&Mkt3&&Mkt3.data&&Mkt3.data.Store&&Mkt3.data.Store.prototype&&Mkt3.data.Store.prototype.sync){var prevDataStoreSync=Mkt3.data.Store.prototype.sync;Mkt3.data.Store.prototype.sync=function(){console.log("Marketo App > Executing: Override Saving for Nurture Streams (sync)");if(window.location.href.search("\/#"+mktoCalendarFragment)!=-1){Mkt3.data.Store.prototype.sync=prevDataStoreSync;}else{var disable;if(typeof(MktCanvas)!=="undefined"&&MktCanvas&&MktCanvas.getActiveTab()&&APP.getCookie("toggleState")!="false"){disable=APP.evaluateMenu("button",null,MktCanvas.getActiveTab(),null);}else if(APP.getCookie("toggleState")=="false"){disable=true;}
...
if(typeof(Ext4)!=="undefined"&&Ext4&&Ext4.data&&Ext4.data.Model&&Ext4.data.Model.prototype&&Ext4.data.Model.prototype.destroy){Ext4.data.Model.prototype.destroy=function(options){console.log("Marketo App > Executing: Override Saving for Nurture Streams (destroy)");var disable;if(typeof(MktCanvas)!=="undefined"&&MktCanvas&&MktCanvas.getActiveTab()&&APP.getCookie("toggleState")!="false"){disable=APP.evaluateMenu("button",null,MktCanvas.getActiveTab(),null);}else if(APP.getCookie("toggleState")=="false"){disable=true;}
...
return me;}else{console.log("Marketo App > Disabling: Saving for Nurture Streams (destroy)");}};}};APP.enableSaving=function(){console.log("Marketo App > Enabling: Saving for Editors");if(typeof(Mkt3)!=="undefined"&&Mkt3&&Mkt3.data&&Mkt3.data.Store&&Mkt3.data.Store.prototype&&Mkt3.data.Store.prototype.sync){Mkt3.data.Store.prototype.sync=function(){console.log("Marketo App > Executing: Enable Saving for Editors (sync)");if(this.autoSyncSuspended){this.autoSync=true;this.autoSyncSuspended=false;}
...
var canDiscardDrafts=window.setInterval(function(){if(typeof(mktLPLManager)!=="undefined"&&mktLPLManager){window.clearInterval(canDiscardDrafts);APP.discardLandingPageDrafts(lpIds);var canDiscardEmailDrafts=window.setInterval(function(){if(typeof(mktEmManager)!=="undefined"&&mktEmManager.discardDraft){window.clearInterval(canDiscardEmailDrafts);APP.discardEmailDrafts(emIds);}},0);var canDiscardOtherDrafts=window.setInterval(function(){if(typeof(Ext4)!=="undefined"&&Ext4&&Ext4.getStore&&Ext4.create&&typeof(Mkt3)!=="undefined"&&Mkt3){window.clearInterval(canDiscardOtherDrafts);APP.discardOtherDrafts("Form",formIds);APP.discardOtherDrafts("MobilePushNotification",pushIds);APP.discardOtherDrafts("InAppMessage",inAppIds);APP.discardOtherDrafts("SocialApp",socIds);}},0);APP.limitNurturePrograms();}},0);}
APP.trackTreeNodeSelection=function(){console.log("Marketo App > Tracking: Tree Node Selection");if(typeof(MktExplorer)!=="undefined"&&MktExplorer&&MktExplorer.selectTreeNode){MktExplorer.selectTreeNode=function(node){var currNode=node,heapEventName,heapEvent={assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,workspaceId:currNode.attributes.accessZoneId};if(currNode.attributes.accessZoneId!=mktoUserWorkspaceId){heapEventName=currNode.text;for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" | "+heapEventName;}}else{heapEventName=userWorkspaceName+" | "+userName;}
heapEvent.name=heapEventName;heapTrack("track",heapEvent);if(!node||!this.boundTree)
return false;if(!node.isSelected()){this.boundTree.expandPath(node.getPath(),'id');node.render();node.select();return true;}else{return false;}};}};APP.trackOtherAssets=function(){console.log("Marketo App > Tracking: Social App or Push Notification Selection");if(typeof(MktExplorer)!=="undefined"&&MktExplorer&&MktExplorer.getNodeById&&MktExplorer.getNodeById(currUrlFragment.substring(0,currUrlFragment.length-5))){var node=currNode=MktExplorer.getNodeById(currUrlFragment.substring(0,currUrlFragment.length-5)),heapEventName,heapEvent={assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,workspaceId:currNode.attributes.accessZoneId};if(currNode.attributes.accessZoneId!=mktoUserWorkspaceId){heapEventName=currNode.text;for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" | "+heapEventName;}}else{heapEventName=userWorkspaceName+" | "+userName;}
