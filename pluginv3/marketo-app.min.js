...
if(attr.folder){if(attr.cancelFirstExpand){delete this.attributes.cancelFirstExpand;}else if(this.childNodes&&this.childNodes.length>0&&!attr.mktExpanded&&this.attributes&&this.attributes.accessZoneId){if(this.attributes.accessZoneId!=mktoUserWorkspaceId){MktFolder.saveExpandState(this,true);}
else{console.log("Marketo App > NOT Saving: Folder Expand State");}}}
...
heapEvent.workspaceName=APP.getWorkspaceName(currNode.attributes.accessZoneId);if(currNode.attributes.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){heapEvent.name=heapEvent.workspaceName;if(heapEvent.workspaceName=="Admin"){heapEvent.workspaceId=0;}}else if(currNode.attributes.accessZoneId==mktoUserWorkspaceId){heapEvent.name=heapEvent.workspaceName;heapEvent.userFolder=userName;}else{heapEvent.name="User's Workspace"}
...
if(event){heapEventProps={app:heapApp,assetName:event.assetName,assetId:event.assetId,assetType:event.assetType,assetPath:event.assetPath,workspaceId:event.workspaceId,workspaceName:event.workspaceName,area:heapArea,url:currentUrl};console.log("Marketo App > Tracking: Heap Event: "+event.name+"\n"+JSON.stringify(heapEventProps,null,2));heap.track(event.name,heapEventProps);}else{if(typeof(MktCanvas)!=="undefined"&&MktCanvas&&MktCanvas.getActiveTab()&&MktCanvas.getActiveTab().config&&MktCanvas.getActiveTab().config.accessZoneId){heapEventProps={app:heapApp,assetId:activeTabConfig.expNodeId,assetType:activeTabConfig.compType,workspaceId:activeTabConfig.accessZoneId,workspaceName:"",area:heapArea,url:currentUrl};var activeTabConfig=MktCanvas.getActiveTab().config,heapEventTitle=heapEventProps.workspaceName=APP.getWorkspaceName(activeTabConfig.accessZoneId);if(typeof(MktExplorer)!=="undefined"&&MktExplorer&&MktExplorer.getNodeById&&heapEventProps.assetId){var currNode=MktExplorer.getNodeById(heapEventProps.assetId);heapEventProps.assetName=heapEventProps.assetPath=currNode.text;for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventProps.assetPath=currNode.text+" > "+heapEventProps.assetPath;}}
