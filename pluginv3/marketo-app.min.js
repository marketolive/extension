...
APP.trackNodeClick=function(){console.log("Marketo App > Tracking: Tree Node Click");if(typeof(Ext)!=="undefined"&&Ext&&Ext.tree&&Ext.tree.TreeEventModel&&Ext.tree.TreeEventModel.prototype&&Ext.tree.TreeEventModel.prototype.onNodeClick){Ext.tree.TreeEventModel.prototype.onNodeClick=function(e,node){if(node&&node.text&&node.attributes&&node.attributes.accessZoneId){var currNode=node,heapEventName,heapEvent={name:"",assetArea:"",assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,assetPath:"",workspaceId:currNode.attributes.accessZoneId,workspaceName:""};if(MktPage&&MktPage.baseTitle){heapEvent.assetArea=MktPage.baseTitle.split("•")[0].trimRight();}else{heapEvent.assetArea="Unknown";}
if(currNode.attributes.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){heapEvent.assetPath=currNode.text;for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEvent.assetPath=currNode.text+" > "+heapEvent.assetPath;}
heapEvent.workspaceName=currNode.text;heapEventName=heapEvent.workspaceName+" > "+heapEvent.assetArea+" > "+heapEvent.assetType;}else if(currNode.attributes.accessZoneId==mktoUserWorkspaceId){heapEventName=userWorkspaceName+" > "+userName;heapEvent.workspaceName=userWorkspaceName;}else{for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;}
