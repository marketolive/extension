...
else{if(this.currNode.parentNode.attributes.compType.search("Folder$")!=-1&&value.toLowerCase().search(userName+"$")==-1){value=value+" - "+userName;if(folder){folder.setText(value);}}
...
else{if(text.toLowerCase().search(userName+"$")==-1){text=text+" - "+userName;MktFolder.currNode.setText(text);}
...
var prevLeadDatabaseShowContextMenu=MktLeadDbMenu.showContextMenu;MktLeadDbMenu.showContextMenu=function(node,e){prevLeadDatabaseShowContextMenu.apply(this,arguments);var compType,menu;var attr=node.attributes;var xtraAttr=attr.xtra;var type;var isTempNode=Ext.isString(node.id)?(node.id.indexOf('xnode-')>-1):false;if(!isTempNode){if(attr.nodeType=='Marketing Folder'){return;}else if(xtraAttr&&xtraAttr.type){type=xtraAttr.type;}else if((attr&&attr.folder)||attr.nodeType=='List'||attr.nodeType=='Smart List'||attr.nodeType=='Segmentation'){type='LeadDb';}else{return;}
menu=MktMenuMgr.getMenu(type,attr);debugger;var userId=MktPage.userid.toLowerCase(),userName;if(userId.search("\.demo@marketo.com$")!=-1){userName=userId.split(".demo")[0];}
else{userName=userId.split("@")[0];}
if(menu&&menu.items){var mItems=menu.items,canvas=MktCanvas.getActiveTab(),disable=(attr&&attr.accessZoneId&&(attr.accessZoneId==1||attr.accessZoneId==mktoJapaneseWorkspaceId||(attr.accessZoneId==mktoMarketingWorkspaceId&&(attr.system==true||(attr.menu.currNode.parentNode.attributes.system==true&&attr.text!=userName)))))||(attr&&attr.depth&&attr.depth==1&&attr.accessZoneId&&attr.accessZoneId==mktoMarketingWorkspaceId)||(!attr||!attr.accessZoneId&&menu&&menu.currNode&&menu.currNode.attributes&&menu.currNode.attributes.accessZoneId&&(menu.currNode.attributes.accessZoneId==1||menu.currNode.attributes.accessZoneId==mktoJapaneseWorkspaceId||(menu.currNode.attributes.accessZoneId==mktoMarketingWorkspaceId&&(menu.currNode.attributes.system==true||(menu.currNode.parentNode.attributes.system==true&&menu.currNode.text!=userName)))))||(!menu||!menu.currNode&&canvas&&canvas.config&&canvas.config.accessZoneId&&(canvas.config.accessZoneId==1||canvas.config.accessZoneId==mktoJapaneseWorkspaceId)),itemsToDisable=["newSmartList","newList","importList","cloneSmartlist","cloneList","deleteList","newSegmentation","createDraftSegmentation","approveSegmentation","unapproveSegmentation","deleteSegmentation","refreshSegmentation","approveDraftSegmentation","share","createFolder","renameFolder","deleteFolder","convertToArchiveFolder","convertToFolder",];itemsToDisable.forEach(function(itemToDisable){var item=mItems.get(itemToDisable);if(item){item.setDisabled(disable);}});}
