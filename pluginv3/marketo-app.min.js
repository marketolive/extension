...
if(currUrlFragment.search("^"+mktoEmailDesignerFragment)==-1&&currUrlFragment.search("^"+mktoEmailPreviewFragment)&&currUrlFragment.search("^"+mktoLandingPageDesignerFragment)==-1&&currUrlFragment.search("^"+mktoLandingPagePreviewFragment)==-1&&currUrlFragment.search("^"+mktoFormWizardFragment)==-1&&currUrlFragment.search("^"+mktoMobilePushNotificationWizardFragment)==-1&&currUrlFragment.search("^"+mktoSocialAppWizardFragment)==-1){if(currUrlFragment!=mktoMyMarketoFragment){var isMktCanvas=window.setInterval(function(){if(MktCanvas.activeTab!==null){console.log("Marketo App > Location: Marketo Canvas");window.clearInterval(isMktCanvas);prevWorkspaceId=MktCanvas.activeTab.config.accessZoneId;if(prevWorkspaceId==1){APP.disableSmartCampaignSaving();APP.enableSmartCampaignCanvas();APP.disableSaving();}}},0);var isMktTree=window.setInterval(function(){if(typeof(document.getElementById("ProgramsTree"))!=="undefined"){console.log("Marketo App > Location: Marketo Tree");window.clearInterval(isMktTree);APP.hideFolders(username);}},0);}
...
prevWorkspaceId=currWorkspaceId;}}},0);var isMktTree=window.setInterval(function(){if(typeof(document.getElementById("ProgramsTree"))!=="undefined"){console.log("Marketo App > Location: Marketo Tree");window.clearInterval(isMktTree);APP.hideFolders(username);}},0);if(currUrlFragment==oppInfluenceAnalyzerFragment||currUrlFragment==programAnalyzerFragment||currUrlFragment==modeler106Fragment||currUrlFragment==modeler106abFragment||currUrlFragment==successPathAnalyzerFragment){console.log("Marketo App > Location: Analytics");APP.injectAnalyzerNavBar();}}}}
