...
APP.evaluateMenu=function(triggeredFrom,attr,menu,canvas,toolbar){console.log("Marketo App > Evaluating: Menu");var userName,toBeDisabled=true,userId=MktPage.userid.toLowerCase();if(userId.search("\.demo@marketo.com$")!=-1){userName=userId.split(".demo")[0];}
else{userName=userId.split("@")[0];}
switch(triggeredFrom){case"tree":if((attr&&(attr.accessZoneId==1||attr.accessZoneId==mktoJapaneseWorkspaceId||(attr.accessZoneId==mktoMarketingWorkspaceId&&((attr.id&&MktExplorer.getNodeById(attr.id))||(attr.menu&&attr.menu.currNode)))))){if(attr.accessZoneId==mktoMarketingWorkspaceId){if(MktExplorer.getNodeById(attr.id)){currNode=MktExplorer.getNodeById(attr.id);}
