...
APP.overrideTreeNodeExpand=function(){console.log("Marketo App > Overriding: Tree Node Expand");MktAsyncTreeNode.prototype.expand=function(){var attr=this.attributes,userWorkspaceName="Marketing",userName=MktPage.userid.split("@")[0],ii;if(attr.folder){if(attr.cancelFirstExpand){delete this.attributes.cancelFirstExpand;}
else if(this.childNodes&&this.childNodes.length>0&&!attr.mktExpanded){MktFolder.saveExpandState(this,true);}}
if(this.text==userWorkspaceName||this.parentNode.text==userWorkspaceName){for(ii=0;ii<this.childNodes.length;ii++){if(this.childNodes[ii].attributes.system==false){if(this.childNodes[ii].text!==userName){APP.hideFolders(this.childNodes[ii].ui.iconNode.id);}}}}
MktAsyncTreeNode.superclass.expand.apply(this,arguments);attr.mktExpanded=true;}}
APP.hideFolders=function(folderId){console.log("Marketo App > Hiding: Folders");document.getElementById(folderId).parentElement.setAttribute("style","display:none");}
