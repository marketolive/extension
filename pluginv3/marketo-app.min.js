...
isEmailEditor2=window.setInterval(function(){if(action=="edit"){if(overlay(document.getElementsByTagName("iframe")[0].contentWindow.document)==true){window.clearInterval(isEmailEditor2);}}
else if(action=="preview"){emailDocuments=document.getElementsByTagName("iframe");for(ii=0;ii<emailDocuments.length;ii++){currEmailDocument=emailDocuments[ii];if(currEmailDocument.getAttribute("id").search("^iframeComponent-")!=-1){numOfIframes++;if(overlay(currEmailDocument)==true){numOfIframesReplaced++;}}}
if(numOfIframesReplaced==numOfIframes){window.clearInterval(isEmailEditor2);}}},0);}
