...
isEmailEditor2=window.setInterval(function(){if(action=="edit"){console.log("Marketo App > Overlaying: Email Editor");if(overlay(document.getElementsByTagName("iframe")[0].contentWindow.document)==true){console.log("Marketo App > Overlaying: Email Interval is Cleared");window.clearInterval(isEmailEditor2);}}
else if(action=="preview"){console.log("Marketo App > Overlaying: Email Previewer");emailIframes=document.getElementsByTagName("iframe");for(ii=0;ii<emailIframes.length;ii++){currEmailIframe=emailIframes[ii];if(currEmailIframe.getAttribute("id").search("^iframeComponent-")!=-1){numOfIframes++;if(overlay(currEmailIframe.contentWindow.document)==true){numOfIframesReplaced++;}}}
