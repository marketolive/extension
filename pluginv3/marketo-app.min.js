...
overlay=function(emailDocument){if(emailDocument!=null){emailBody=emailDocument.getElementsByTagName("body")[0];mktoImgs=emailBody.getElementsByClassName("mktoImg");mktoTexts=emailBody.getElementsByClassName("mktoText");mktoButtons=emailBody.getElementsByClassName("secondary-font button");logoSwapCompany=emailDocument.getElementById("logo-swap-company");logoSwapContainer=emailDocument.getElementById("logo-swap-container");logoSwapCompanyContainer=emailDocument.getElementById("logo-swap-company-container");logoBkg=emailDocument.getElementById("logo-bkg");buttonBkg=emailDocument.getElementById("button-bkg");if(mktoImgs&&!isMktoImgReplaced){for(ii=0;ii<mktoImgs.length;ii++){currMktoImg=mktoImgs[ii];if(currMktoImg.getAttribute("mktoname").search(logoMktoNameRegex)!=-1){console.log("Marketo App > Overlaying: Email 2.0 Company Logo");currMktoImg.getElementsByTagName("img")[0].setAttribute("src",logo);isMktoImgReplaced=true;break;}}}
if(mktoTexts&&!isMktoSubTextReplaced&&!isMktoTextReplaced){for(ii=0;ii<mktoTexts.length;ii++){currMktoText=mktoTexts[ii];if(currMktoText.getAttribute("mktoname").search(mainTitleMktoNameRegex)!=-1){console.log("Marketo App > Overlaying: Email 2.0 Company Name");currMktoText.innerHTML=companyName+" Invites You<br><br>PREMIER BUSINESS EVENT<br>OF THE YEAR";isMktoTextReplaced=true;}
else if(currMktoText.getAttribute("mktoname").search(subTitleMktoNameRegex)!=-1){console.log("Marketo App > Overlaying: Email 2.0 Today's Date");currMktoText.innerHTML=dayOfWeek+", "+month+" the "+dayOfMonth+" "+year;isMktoSubTextReplaced=true;}
if(isMktoSubTextReplaced&&isMktoTextReplaced){break;}}}
if(mktoButtons&&!isMktoButtonReplaced){for(ii=0;ii<mktoButtons.length;ii++){currMktoButton=mktoButtons[ii];if(currMktoButton.innerHTML.search(buttonTextRegex)!=-1){console.log("Marketo App > Overlaying: Email 2.0 Company Color");currMktoButton.style.backgroundColor=color;isMktoButtonReplaced=true;break;}}}
