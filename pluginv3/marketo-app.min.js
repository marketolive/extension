...
if((isMktoButtonReplaced&&isMktoSubTextReplaced&&isMktoTextReplaced&&isMktoImgReplaced)||isMktoEmail1Replaced){window.clearInterval(isEmailEditor2);}}
if(action=="edit"){overlay(document.getElementsByTagName("iframe")[0].contentWindow.document);}
else if(action=="preview"){emailDocuments=document.getElementsByTagName("iframe");for(ii=0;ii<emailDocuments.length;ii++){currEmailDocument=emailDocuments[ii];if(currEmailDocument.getAttribute("id").search("^iframeComponent-")!=-1){overlay(currEmailDocument);}}}},0);}
