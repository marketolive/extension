...
if(attr.folder){if(attr.cancelFirstExpand){delete this.attributes.cancelFirstExpand;}else if(this.childNodes&&this.childNodes.length>0&&!attr.mktExpanded&&this.attributes&&this.attributes.accessZoneId){if(this.accessZoneId!=mktoUserWorkspaceId){console.log("Marketo App > Saving: Folder Expand State");MktFolder.saveExpandState(this,true);}}}
...
heapEvent.workspaceName=APP.getWorkspaceName(currNode.accessZoneId);if(currNode.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){heapEvent.name=heapEvent.workspaceName;if(heapEvent.workspaceName=="Admin"){heapEvent.workspaceId=0;}}else if(currNode.accessZoneId==mktoUserWorkspaceId){heapEvent.name=heapEvent.workspaceName;heapEvent.userFolder=userName;}else{heapEvent.name="User's Workspace"}
