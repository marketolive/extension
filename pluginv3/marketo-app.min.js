...
APP.disableDemoPluginCheck();if(currUrlFragment){if(currUrlFragment==mktoMyMarketoFragment){APP.overrideHomeTiles();APP.heapTrack("track",{name:"My Marketo",assetName:"Home"});}else if(currUrlFragment.search(mktoDisableButtonsFragmentMatch)!=-1){APP.disableButtons();}else if(currUrlFragment==mktoAnalyticsDefaultFragment){APP.overrideAnalyticsTiles();}else if(currUrlFragment==mktoAdBridgeSmartListFragment){console.log("Marketo App > Location: Ad Bridge Smart List");APP.openAdBridgeModal();}else if(currUrlFragment==mktoAdminSalesforceFragment){console.log("Marketo App > Location: Admin > Salesforce");APP.disableRequests();APP.hideOtherToolbarItems([{id:"enableSync",action:"setVisible"}]);}else if(currUrlFragment==mktoAdminRcaCustomFieldSync){console.log("Marketo App > Location: Admin > Revenue Cycle Analytics > Custom Field Sync");APP.disableRequests();APP.hideOtherToolbarItems([{id:"cadChangeButton",action:"setVisible"}]);}}
