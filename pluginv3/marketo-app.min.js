...
APP.overrideDeliverabilityToolsTile=function(){console.log("Marketo App > Overriding: Deliverability Tools Tile");if(MktCanvas&&MktCanvas.getEl()&&MktCanvas.getEl().dom&&MktCanvas.getEl().dom.nextSibling&&MktCanvas.getEl().dom.nextSibling.childNodes&&MktCanvas.getEl().dom.nextSibling.childNodes[0]&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0]&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0]&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0].childNodes&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0].childNodes[0]&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0]&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0]&&MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes){console.log("Marketo App > Executing: Override Deliverability Tools Tile");var tiles=MktCanvas.getEl().dom.nextSibling.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes,currTileSpanTags,currTileAtags,spareTile,deliverabilityToolsTile,ii,jj;for(ii=0;ii<tiles.length;ii++){if(tiles[ii]&&tiles[ii].getAttribute("role")=="presentation"){spareTile=tiles[ii];break;}
currTileSpanTags=tiles[ii].getElementsByTagName("span");for(jj=0;jj<currTileSpanTags.length;jj++){if(currTileSpanTags[jj]&&currTileSpanTags[jj].innerHTML=="Deliverability Tools"){deliverabilityToolsTile=tiles[ii];break;}}
currTileAtags=tiles[ii].getElementsByTagName("a");for(jj=0;jj<currTileAtags.length;jj++){if(currTileAtags[jj]&&currTileAtags[jj].href.search("/homepage/sso\\?sso=250ok$")!=-1){currTileAtags[jj].href="https://250ok.com/login";break;}}}
...
if(currWorkspaceId==mktoDefaultWorkspaceId||currWorkspaceId==mktoJapaneseWorkspaceId){var toUpdateNodeText=false;MktSession.clockCursor(true);this.getTitleField().setValue(this.titleValue);var canvasTab=MktCanvas.getActiveTab();var nodeId=null;if(canvasTab.config.expNodeId){var node=MktExplorer.getNodeById(canvasTab.config.expNodeId);if(node&&node.attributes.compType){var compType=node.attributes.compType;if(compType=='Marketing Program'){nodeId=canvasTab.config.expNodeId;}
if(compType=='Image'){toUpdateNodeText=false;}}
if(toUpdateNodeText){}}
var el=this.getEl();var panelObj=this;var formPanel=this.formPanel;var viewPanel=this.viewPanel;formPanel.hide(true,0.2);viewPanel.show(true,0.2);viewPanel.body.update(panelObj.viewTemplate.apply(panelObj));el.animate({height:{from:this.getHeight(),to:this.origHeight}},0.25,function(){panelObj.setHeight(panelObj.origHeight);panelObj.body.setHeight(panelObj.origHeight);if(isFunction(panelObj.savedCallback)){panelObj.savedCallback();}});MktSession.unclockCursor();this._saveInProgress=false;}}}}}
...
itemsToDisable.forEach(function(itemToDisable){var item=mItems.get(itemToDisable);if(item){item.setDisabled(disable);}});if(this.ownerCt&&this.ownerCt.text.search("^View:")!=-1){var ii;for(ii=0;ii<this.items.items.length;ii++){switch(this.items.items[ii].text){case"Create View":this.items.items[ii].setDisabled(true);break;case"Edit Default":this.items.items[ii].setDisabled(true);break;default:break;}}}
...
APP.overrideSaving=function(){console.log("Marketo App > Overriding: Saving for Nurture Streams");if(Mkt3&&Mkt3.data&&Mkt3.data.Store&&Mkt3.data.Store.prototype&&Mkt3.data.Store.prototype.sync){Mkt3.data.Store.prototype.sync=function(){console.log("Marketo App > Executing: Override Saving for Nurture Streams (sync)");var disable;if(MktCanvas&&MktCanvas.getActiveTab()&&APP.getCookie("priv")!="false"){disable=APP.evaluateMenu("button",null,MktCanvas.getActiveTab(),null);}
else if(APP.getCookie("priv")=="false"){disable=true;}
...
if(Ext4&&Ext4.data&&Ext4.data.Model&&Ext4.data.Model.prototype&&Ext4.data.Model.prototype.destroy){Ext4.data.Model.prototype.destroy=function(options){console.log("Marketo App > Executing: Override Saving for Nurture Streams (destroy)");var disable;if(MktCanvas&&MktCanvas.getActiveTab()&&APP.getCookie("priv")!="false"){disable=APP.evaluateMenu("button",null,MktCanvas.getActiveTab(),null);}
else if(APP.getCookie("priv")=="false"){disable=true;}
...
var isEmailIframeElement=window.setInterval(function(){var logoBkg=document.getElementsByTagName("iframe")[0].contentWindow.document.getElementById("logo-bkg"),logoSwapCompany=document.getElementsByTagName("iframe")[0].contentWindow.document.getElementById("logo-swap-company"),logoSwapContainer=document.getElementsByTagName("iframe")[0].contentWindow.document.getElementById("logo-swap-container"),logoSwapCompanyContainer=document.getElementsByTagName("iframe")[0].contentWindow.document.getElementById("logo-swap-company-container");if(logoBkg!=null&&logoSwapCompany!=null){console.log("Marketo App > Overlaying: iframe");window.clearInterval(isEmailIframeElement);logoSwapContainer.style.display="none";logoSwapCompanyContainer.style.display="block";logoSwapCompany.src=logo;}},0);}
...
else{var company=logo.split("https://logo.clearbit.com/")[1].split(".")[0],companyName=company.charAt(0).toUpperCase()+company.slice(1);}
if(color==null){color=defaultColor;}
...
return emIds;}
if((currentUrl.search(mktoAppDomain)!=-1||currentUrl.search(mktoDesignerDomain)!=-1||currentUrl.search(mktoWizard)!=-1)&&currentUrl.search("#CAL")==-1){console.log("Marketo App > Location: Marketo URL");var isMktPageApp=window.setInterval(function(){if(typeof(MktPage)!=="undefined"){console.log("Marketo App > Location: Marketo Page");var accountString,userId,currUrlFragment,currCompCode;if(MktPage.savedState&&MktPage.savedState.custPrefix&&MktPage.userid&&Mkt3&&Mkt3.DL&&Mkt3.DL.getDlToken()&&Mkt3.DL.dl&&Mkt3.DL.dl.dlCompCode){window.clearInterval(isMktPageApp);accountString=MktPage.savedState.custPrefix,userId=MktPage.userid.toLowerCase(),currUrlFragment=Mkt3.DL.getDlToken(),currCompFragment=Mkt3.DL.dl.dlCompCode;if(userId.search("\.demo@marketo.com$")!=-1){userName=userId.split(".demo")[0];}
else{userName=userId.split("@")[0];}}
if(accountString==mktoAccountString106||accountString==mktoAccountString106a||accountString==mktoAccountString106b){console.log("Marketo App > Location: MarketoLive Instance");window.mkto_live_plugin_state=true;if(userId.search("^admin@mktodemoaccount")!=-1||userId.search("^mktodemoaccount[a-z0-9]*@marketo\.com")!=-1||userId.search("^marketodemo.*@gmail\.com$")!=-1){console.log("Marketo App > User: Admin");APP.disableDemoPluginCheck();if(APP.getCookie("priv")!="false"){return;}
else{console.log("Marketo App > User: Admin is now a normal user");}}
APP.disableDemoPluginCheck();if(currUrlFragment==mktoMyMarketoFragment){APP.overrideDeliverabilityToolsTile();}
...
else if(currCompFragment==mktoLandingPageDesignerFragment){console.log("Marketo App > Location: Landing Page Designer");var customCompanyLandingPage106Fragment="LPE11826",customCompanyLandingPagePreview106Fragment="LPP11826",customCompanyLandingPage106aFragment="LPE10672",customCompanyLandingPagePreview106aFragment="LPP10672",customCompanyLandingPage106bFragment="LPE10768",customCompanyLandingPagePreview106bFragment="LPP10768",lpParameters={filters:[{property:'id',value:Mkt3.DL.dl.compId}],callback:function(records){records.forEach(function(record){if(record.get('zoneId')){var currAssetWorkspaceId=record.get('zoneId');console.log("Marketo App > currAssetWorkspaceId = "+currAssetWorkspaceId);if(currAssetWorkspaceId==mktoDefaultWorkspaceId||currAssetWorkspaceId==mktoJapaneseWorkspaceId||APP.getCookie("priv")=="false"){APP.disablePropertyPanelSaving();}}});}};if(Ext4&&Ext4.getStore('LandingPage')&&Ext4.getStore('LandingPage').load){console.log("Callback for Landing Page Editor");Ext4.getStore('LandingPage').load(lpParameters);}
...
return;}
else{console.log("Marketo App > Location: Designers/Wizards");var customCompanyEmail106Fragment="EME15464",customCompanyEmail106aFragment="EME14240",customCompanyEmail106bFragment="EME13924",customCompanyHealthcareFragment="EME20327",customCompanyFinservFragment="EME19059",customCompanyHigherEdFragment="EME20329",loadParameters={filters:[{property:'id',value:Mkt3.DL.dl.compId}],callback:function(records){records.forEach(function(record){if(record.get('zoneId')){var currAssetWorkspaceId=record.get('zoneId');console.log("Marketo App > currAssetWorkspaceId = "+currAssetWorkspaceId);if(currAssetWorkspaceId==mktoDefaultWorkspaceId||currAssetWorkspaceId==mktoJapaneseWorkspaceId||APP.getCookie("priv")=="false"){APP.disableSaving();}}});}};switch(currCompFragment){case mktoEmailDesignerFragment:if(Ext4&&Ext4.getStore('Email')&&Ext4.getStore('Email').load){console.log("Callback for Email Editor");Ext4.getStore('Email').load(loadParameters);}
switch(currUrlFragment){case customCompanyEmail106Fragment:APP.overlayEmailDesigner();break;case customCompanyHealthcareFragment:APP.overlayEmailDesigner();break;case customCompanyFinservFragment:APP.overlayEmailDesigner();break;case customCompanyHigherEdFragment:APP.overlayEmailDesigner();break;case customCompanyEmail106aFragment:APP.overlayEmailDesigner();break;case customCompanyEmail106bFragment:APP.overlayEmailDesigner();break;default:break;}
...
break;case mktoABtestWizardFragment:var isABtestWizard=window.setInterval(function(){if(Mkt3&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor")&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor()&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record&&(Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoDefaultWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoJapaneseWorkspaceId)){console.log("Marketo App > Location: A/B Test Wizard");window.clearInterval(isABtestWizard);APP.disableSaving();}},0);break;case mktoEmailTestWizardFragment:var isEmailTestWizard=window.setInterval(function(){if(Mkt3&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor")&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor()&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record&&(Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoDefaultWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoJapaneseWorkspaceId)){console.log("Marketo App > Location: Email Test Group Wizard");window.clearInterval(isEmailTestWizard);APP.disableSaving();}},0);break;default:break;}}
window.onhashchange=function(){console.log("Window: Hash Changed");currentUrl=window.location.href;currUrlFragment=Mkt3.DL.getDlToken();currCompFragment=Mkt3.DL.dl.dlCompCode;if(currUrlFragment==mktoMyMarketoFragment){APP.overrideDeliverabilityToolsTile();}
else if(currUrlFragment==mktoMarketingActivitiesDefaultFragment||currUrlFragment==mktoMarketingActivitiesMarketingFragment||currUrlFragment==mktoMarketingActivitiesJapaneseFragment||currUrlFragment==mktoLeadDatabaseDefaultFragment||currUrlFragment==mktoLeadDatabaseMarketingFragment||currUrlFragment==mktoLeadDatabaseJapaneseFragment){APP.disableButtons();}
else if(currUrlFragment==mktoAnalyticsFragment){APP.overrideAnalyticsTiles();}
