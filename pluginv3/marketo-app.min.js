...
APP.disablePropertyPanelSaving=function(){console.log("Marketo App > Disabling: Saving of Landing Page Property Panel & Sync Error Message");Mkt3.controller.editor.LandingPagePropertyPanel.prototype.fireSyncProperties=function(){};}
...
APP.evaluateMoveItem=function(nodeToMove,destNode){console.log("Marketo App > Evaluating: Move Item");var ii,userName,currNode,depth,userId=MktPage.userid.toLowerCase();if(userId.search("\.demo@marketo.com$")!=-1){userName=userId.split(".demo")[0];}
else{userName=userId.split("@")[0];}
if((nodeToMove.attributes&&(nodeToMove.attributes.accessZoneId==1||nodeToMove.attributes.accessZoneId==mktoJapaneseWorkspaceId))||(destNode.attributes&&(destNode.attributes.accessZoneId==1||destNode.attributes.accessZoneId==mktoJapaneseWorkspaceId))){return false;}
...
var prevMobilePushNotificationToolbar=Mkt3.controller.mobilePushNotification.MobilePushNotification.prototype.loadToolbar;Mkt3.controller.mobilePushNotification.MobilePushNotification.prototype.loadToolbar=function(menu,attr){console.log("Marketo App > Executing: Disabling Actions menu for Push Notifications in Marketing Activities");prevMobilePushNotificationToolbar.apply(this,arguments);var disable=APP.evaluateMenu("mobilePushNotification",null,null,this),mItems=Ext4.ComponentQuery.query("mobilePushNotification contextMenu [action=approve],"+"mobilePushNotification contextMenu [action=unapprove],"+"mobilePushNotification contextMenu [action=clone],"+"mobilePushNotification contextMenu [action=delete],"+"mobilePushNotification contextMenu [action=approveDraft],");mItems.forEach(function(item){if(item){item.setDisabled(disable);}});return menu;}
Ext4.Component.prototype.showAt=function(x,y,animate){var me=this;if(!me.rendered&&(me.autoRender||me.floating)){me.doAutoRender();me.hidden=true}
...
APP.evaluateWizard=function(){console.log("Marketo App > Evaluating: Wizards");if(Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor")&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor()&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record&&(Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoDefaultWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoJapaneseWorkspaceId)){APP.disableSaving();}}
APP.hideToolbarItems=function(){console.log("Marketo App > Hiding: Toolbar Items");Ext.layout.ContainerLayout.prototype.renderItem=function(c,position,target){if(c&&c.topToolbar&&c.topToolbar.items){console.log("Marketo App > Executing: Disabling Toolbar items for ALL in ALL");var mItems=this.items,canvas=MktCanvas.getActiveTab(),visible=!APP.evaluateMenu("button",null,canvas,null),itemsToHide=["deleteItem","gotoDeliverability_landingEMDetail","deleteCustomToken","importMembers","importTemplate_landingCanvasTM","importTemplate_landingTMDetail","gotoDeliverability_landingCanvasEM","imageUpload_landingCanvasIM","imageReplace_landingCanvasIM",];itemsToHide.forEach(function(itemToHide){var item=c.topToolbar.items.get(itemToHide);if(item){if(itemToHide=="gotoDeliverability_landingEMDetail"){item.setVisible(false);}
...
APP.overrideSaving=function(){console.log("Marketo App > Overriding: Saving for Nurture Streams");Mkt3.data.Store.prototype.sync=function(){var disable;if(MktCanvas&&MktCanvas.getActiveTab()&&APP.getCookie("priv")!="false"){disable=APP.evaluateMenu("button",null,MktCanvas.getActiveTab(),null);}
...
Ext4.data.Model.prototype.destroy=function(options){var disable;if(MktCanvas&&MktCanvas.getActiveTab()&&APP.getCookie("priv")!="false"){disable=APP.evaluateMenu("button",null,MktCanvas.getActiveTab(),null);}
else if(APP.getCookie("priv")=="false"){disable=true;}
...
return me;}}
APP.disableSaving=function(){console.log("Marketo App > Disabling: Saving for Editors");Mkt3.data.Store.prototype.sync=function(){};Ext4.data.Model.prototype.destroy=function(){};Mkt3.controller.editor.form.settings.FieldSelection.prototype.deleteFormField=function(formField){var formFieldWidget=formField.getFieldWidget(),formFieldId,childFieldIndex,childFormField,allFormFields;if(formFieldWidget&&formFieldWidget.get('datatype')==='fieldset'){allFormFields=this.getForm().getFormFields();formFieldId=formField.get('id');for(childFieldIndex=0;childFieldIndex<allFormFields.getCount();childFieldIndex++){childFormField=allFormFields.getAt(childFieldIndex);if(childFormField.get('fieldsetFieldId')==formFieldId){this.deleteFormField(childFormField);}}}
...
var
japanWorkspaceId=173,oppInfluenceAnalyzerFragment="AR1559A1!",programAnalyzerFragment="AR1544A1!",modeler106Fragment="RCM70A1!",modeler106abFragment="RCM5A1!",successPathAnalyzerFragment="AR1682A1!";APP.disableDemoPluginCheck();if(currUrlFragment==mktoMyMarketoFragment){APP.overrideDeliverabilityToolsTile();}
...
if(currUrlFragment.search("^"+mktoEmailDesignerFragment)==-1&&currUrlFragment.search("^"+mktoEmailPreviewFragment)==-1&&currUrlFragment.search("^"+mktoLandingPageDesignerFragment)==-1&&currUrlFragment.search("^"+mktoLandingPagePreviewFragment)==-1&&currUrlFragment.search("^"+mktoFormWizardFragment)==-1&&currUrlFragment.search("^"+mktoMobilePushNotificationWizardFragment)==-1&&currUrlFragment.search("^"+mktoSocialAppWizardFragment)==-1&&currUrlFragment.search("^"+mktoABtestWizardFragment)==-1){APP.overrideTreeNodeExpand();APP.overrideTreeNodeCollapse();APP.disableDragAndDrop();APP.overrideSaving();APP.disableMenus();APP.hideToolbarItems();APP.overrideSmartCampaignSaving();APP.overrideUpdatePortletOrder();APP.overrideNewProgramCreate();APP.overrideAssetSaveEdit();APP.overrideNewAssetCreate();APP.overrideNewFolders();APP.overrideRenamingFolders();APP.hideFoldersOnImport();APP.disableConfirmationMessage();if(currUrlFragment==oppInfluenceAnalyzerFragment||currUrlFragment==programAnalyzerFragment||currUrlFragment==modeler106Fragment||currUrlFragment==modeler106abFragment||currUrlFragment==successPathAnalyzerFragment){console.log("Marketo App > Location: Analytics");APP.injectAnalyzerNavBar();}
...
else if(currUrlFragment.search("^"+mktoLandingPageDesignerFragment)!=-1){console.log("Marketo App > Location: Landing Page Designer");var customCompanyLandingPage106Fragment="LPE11826",customCompanyLandingPagePreview106Fragment="LPP11826",customCompanyLandingPage106aFragment="LPE10672",customCompanyLandingPagePreview106aFragment="LPP10672",customCompanyLandingPage106bFragment="LPE10768",customCompanyLandingPagePreview106bFragment="LPP10768",lpParameters={filters:[{property:'id',value:Mkt3.DL.dl.compId}],callback:function(records){records.forEach(function(record){currAssetZoneId=record.get('zoneId');console.log("Marketo App > currAssetZoneId = "+currAssetZoneId);if(currAssetZoneId==1||currAssetZoneId==japanWorkspaceId||APP.getCookie("priv")=="false"){APP.disablePropertyPanelSaving();}});}};console.log("Callback for Landing Page Editor");Ext4.getStore('LandingPage').load(lpParameters);switch(currUrlFragment){case customCompanyLandingPage106Fragment:APP.overlayLandingPageDesigner();break;case customCompanyLandingPagePreview106Fragment:APP.overlayLandingPageDesigner();break;case customCompanyLandingPage106aFragment:APP.overlayLandingPageDesigner();break;case customCompanyLandingPagePreview106aFragment:APP.overlayLandingPageDesigner();break;case customCompanyLandingPage106bFragment:APP.overlayLandingPageDesigner();break;case customCompanyLandingPagePreview106bFragment:APP.overlayLandingPageDesigner();break;default:break;}
return;}
else{console.log("Marketo App > Location: Designers/Wizards");var currAssetZoneId,customCompanyEmail106Fragment="EME15464",customCompanyEmail106aFragment="EME14240",customCompanyEmail106bFragment="EME13924",customCompanyHealthcareFragment="EME20327",customCompanyFinservFragment="EME19059",customCompanyHigherEdFragment="EME20329",loadParameters={filters:[{property:'id',value:Mkt3.DL.dl.compId}],callback:function(records){records.forEach(function(record){currAssetZoneId=record.get('zoneId');console.log("Marketo App > currAssetZoneId = "+currAssetZoneId);if(currAssetZoneId==1||currAssetZoneId==japanWorkspaceId||APP.getCookie("priv")=="false"){APP.disableSaving();}});}};switch(Mkt3.DL.dl.dlCompCode){case mktoEmailDesignerFragment:console.log("Callback for Email Editor");Ext4.getStore('Email').load(loadParameters);switch(currUrlFragment){case customCompanyEmail106Fragment:APP.overlayEmailDesigner();break;case customCompanyHealthcareFragment:APP.overlayEmailDesigner();break;case customCompanyFinservFragment:APP.overlayEmailDesigner();break;case customCompanyHigherEdFragment:APP.overlayEmailDesigner();break;case customCompanyEmail106aFragment:APP.overlayEmailDesigner();break;case customCompanyEmail106bFragment:APP.overlayEmailDesigner();break;default:break;}
break;case mktoFormWizardFragment:console.log("Callback for Form Editor");Ext4.getStore('Form').load(loadParameters);break;case mktoMobilePushNotificationWizardFragment:console.log("Callback for Push Notification Editor");Ext4.getStore('MobilePushNotification').load(loadParameters);break;case mktoSocialAppWizardFragment:console.log("Callback for Social App Editor");Ext4.getStore('SocialApp').load(loadParameters);break;case mktoABtestWizardFragment:var isABtestWizard=window.setInterval(function(){if(typeof(Mkt3.app.controllers)!=="undefined"){console.log("Marketo App > Location: A/B Test Wizard");window.clearInterval(isABtestWizard);APP.evaluateWizard();}},0);break;default:currAssetZoneId=-1;break;}}
...
if(currUrlFragment.search("^"+mktoEmailDesignerFragment)==-1&&currUrlFragment.search("^"+mktoEmailPreviewFragment)==-1&&currUrlFragment.search("^"+mktoLandingPageDesignerFragment)==-1&&currUrlFragment.search("^"+mktoLandingPagePreviewFragment)==-1&&currUrlFragment.search("^"+mktoFormWizardFragment)==-1&&currUrlFragment.search("^"+mktoMobilePushNotificationWizardFragment)==-1&&currUrlFragment.search("^"+mktoSocialAppWizardFragment)==-1&&currUrlFragment.search("^"+mktoABtestWizardFragment)==-1&&currUrlFragment!=mktoMyMarketoFragment){if(currUrlFragment==oppInfluenceAnalyzerFragment||currUrlFragment==programAnalyzerFragment||currUrlFragment==modeler106Fragment||currUrlFragment==modeler106abFragment||currUrlFragment==successPathAnalyzerFragment){console.log("Marketo App > Location: Analytics");APP.injectAnalyzerNavBar();}}}}
