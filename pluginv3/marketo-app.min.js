...
var disable=APP.evaluateMenu("inAppMessage",null,null,this),mItems=Ext4.ComponentQuery.query("inAppMessageEditor menu [action=approveAndClose],"+"inAppMessage contextMenu [action=unapprove],"+"inAppMessage contextMenu [action=approve],"+"inAppMessage contextMenu [action=clone],"+"inAppMessage contextMenu [action=delete],"+"inAppMessage contextMenu [action=approveDraft],");mItems.forEach(function(item){if(item){item.setDisabled(disable);}});}}
...
else if(currCompFragment==mktoLandingPageDesignerFragment){console.log("Marketo App > Location: Landing Page Editor");var customCompanyLandingPage106Fragment="LPE11826",customCompanyLandingPagePreview106Fragment="LPP11826",customCompanyLandingPage106aFragment="LPE10672",customCompanyLandingPagePreview106aFragment="LPP10672",customCompanyLandingPage106bFragment="LPE10768",customCompanyLandingPagePreview106bFragment="LPP10768";lpParameters={filters:[{property:'id',value:Mkt3.DL.dl.compId}],callback:function(records){records.forEach(function(record){if(record.get('zoneId')){var currAssetWorkspaceId=record.get('zoneId');console.log("Marketo App > currAssetWorkspaceId = "+currAssetWorkspaceId);if(currAssetWorkspaceId==mktoDefaultWorkspaceId||currAssetWorkspaceId==mktoJapaneseWorkspaceId||currAssetWorkspaceId==mktoFinservWorkspaceId||currAssetWorkspaceId==mktoHealthcareWorkspaceId||currAssetWorkspaceId==mktoHigherEdWorkspaceId||APP.getCookie("toggleState")=="false"){if(Ext4&&Ext4.ComponentQuery&&Ext4.ComponentQuery.query){var mItems=Ext4.ComponentQuery.query("lpEditor menu [action=approveAndClose],"+"lpEditor menu [action=disableMobileVersion],"+"lpEditor menu [action=uploadImage],"+"lpEditor menu [action=grabImages],");if(mItems){console.log("Marketo App > Disabling Landing Page Editor Toolbar Actions menus");mItems.forEach(function(item){if(item){item.setDisabled(true);}});}}}}});}};if(Ext4&&Ext4.getStore('LandingPage')&&Ext4.getStore('LandingPage').load){console.log("Callback for Landing Page Editor");Ext4.getStore('LandingPage').load(lpParameters);}
