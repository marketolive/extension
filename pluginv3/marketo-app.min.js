...
else{console.log("Marketo App > Location: Designers/Wizards");var loadParameters={filters:[{property:'id',value:Mkt3.DL.dl.compId}],callback:function(records){records.forEach(function(record){if(record.getNodeJson()){var assetNode=record.getNodeJson(),heapEvent={name:assetNode.text,assetType:assetNode.compType
assetId:assetNode.id,workspaceId:assetNode.accessZoneId};if(assetNode.text.search(".")!=-1){heapEvent.assetName=assetNode.text.split(".")[1];}
else{heapEvent.assetName=assetNode.text;}
heap("track",heapEvent);}
