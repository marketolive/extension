...
APP.disableDemoPluginCheck();if(currUrlFragment==mktoMyMarketoFragment){APP.overrideHomeTiles();heapTrack("track",{name:"My Marketo",assetName:"Home"});}else if(currUrlFragment.search(mktoDisableButtonsFragmentMatch)!=-1){APP.disableButtons();}else if(currUrlFragment==mktoAnalyticsDefaultFragment){APP.overrideAnalyticsTiles();}else if(currUrlFragment.search(mktoOtherAssetsFragmentMatch)!=-1){APP.trackOtherAssets();}else if(currUrlFragment.search(mktoAnalyzersFragmentMatch)!=-1){console.log("Marketo App > Location: Analytics");APP.injectAnalyzerNavBar();}else if(currUrlFragment==mktoAdBridgeSmartListFragment){console.log("Marketo App > Location: Ad Bridge Smart List");APP.openAdBridgeModal();}else if(currUrlFragment==mktoAdminSalesforceFragment){console.log("Marketo App > Location: Admin > Salesforce");APP.hideOtherToolbarItems([{id:"enableSync",action:"setVisible"}]);}else if(currUrlFragment==mktoAdminRcaCustomFieldSync){console.log("Marketo App > Location: Admin > Revenue Cycle Analytics > Custom Field Sync");APP.hideOtherToolbarItems([{id:"cadChangeButton",action:"setVisible"}]);}
if(!currCompFragment||currCompFragment.search(mktoDesignersFragmentMatch)==-1){if(accountString.search(mktoAccountStringsMatch)!=-1){APP.discardDrafts(accountString,"landingPage");APP.overrideTreeNodeExpand();APP.overrideTreeNodeCollapse();APP.overrideSaving();APP.disableDragAndDrop();APP.disableMenus();APP.hideToolbarItems();APP.disableFormSaveButtons();APP.disableAdminSaveButtons();APP.overrideSmartCampaignSaving();APP.trackTreeNodeSelection();APP.overrideUpdatePortletOrder();APP.overrideNewProgramCreate();APP.overrideAssetSaveEdit();APP.overrideNewAssetCreate();APP.overrideNewFolders();APP.overrideRenamingFolders();APP.limitNurturePrograms();APP.hideFoldersOnImport();APP.disableConfirmationMessage();}else{APP.overrideSaving();APP.overrideSmartCampaignSaving();APP.overrideUpdatePortletOrder();APP.disableConfirmationMessage();}
...
window.onhashchange=function(){console.log("Marketo App > Window: Hash Changed");currentUrl=window.location.href;var isNewUrlFragment=window.setInterval(function(){if(typeof(Mkt3)!=="undefined"&&Mkt3&&Mkt3.DL&&Mkt3.DL.getDlToken()){if(currUrlFragment!=Mkt3.DL.getDlToken()){window.clearInterval(isNewUrlFragment);currUrlFragment=Mkt3.DL.getDlToken();console.log("Marketo App > Loaded: New URL Fragment = "+currUrlFragment);if(currUrlFragment==mktoMyMarketoFragment){APP.overrideHomeTiles();heapTrack("track",{name:"My Marketo",assetName:"Home"});}else if(currUrlFragment.search(mktoDisableButtonsFragmentMatch)!=-1){APP.disableButtons();}else if(currUrlFragment==mktoAnalyticsDefaultFragment){APP.overrideAnalyticsTiles();}else if(currUrlFragment.search(mktoOtherAssetsFragmentMatch)!=-1){APP.trackOtherAssets();}else if(currUrlFragment.search(mktoAnalyzersFragmentMatch)!=-1){console.log("Marketo App > Location: Analytics");APP.injectAnalyzerNavBar();}else if(currUrlFragment==mktoAdminSalesforceFragment){console.log("Marketo App > Location: Admin > Salesforce");APP.hideOtherToolbarItems([{id:"enableSync",action:"setVisible"}]);}else if(currUrlFragment==mktoAdminRcaCustomFieldSync){console.log("Marketo App > Location: Admin > Revenue Cycle Analytics > Custom Field Sync");APP.hideOtherToolbarItems([{id:"cadChangeButton",action:"setVisible"}]);}
