...
APP.hideToolbarItems=function(){console.log("Marketo App > Hiding: Toolbar Items");if(Ext&&Ext.layout&&Ext.layout.ContainerLayout&&Ext.layout.ContainerLayout.prototype&&Ext.layout.ContainerLayout.prototype.renderItem){Ext.layout.ContainerLayout.prototype.renderItem=function(c,position,target){if(c&&c.topToolbar&&c.topToolbar.items){console.log("Marketo App > Executing: Disable Toolbar items for ALL in ALL");var item,canvas=MktCanvas.getActiveTab(),visible=!APP.evaluateMenu("button",null,canvas,null),itemsToHide=["deleteItem","gotoDeliverability_landingEMDetail","deleteCustomToken","importMembers","importTemplate_landingCanvasTM","importTemplate_landingTMDetail","gotoDeliverability_landingCanvasEM","imageUpload_landingCanvasIM","imageReplace_landingCanvasIM","Invite New User","caadEditButton","caadEditSecurityButton","caadEditRestrictedLoginButton","Invite New User","editLicenses","editUser","deleteUser","resetPassword","editRole","deleteRole","editZone","deleteZone","editPartition","deletePartition","Edit IP Settings","Edit Text Editor Settings","Edit Email Editor Settings","addDomain","deleteDomain","refreshCadSfdcObjectSync","editVisibleFieldsCadSfdcObjectSync","enableSync","Edit API Configuration","Edit Settings","issueLicenseCadLisAdmin","revokeLicenseCadLisAdmin","resendLicenseCadLisAdmin","configAddinCadLisAdmin","editRule","deleteRule","editIpRestriction","editWebinarLogin","cadChangeButton",];itemsToHide.forEach(function(itemToHide){item=c.topToolbar.items.get(itemToHide);if(item){if(itemToHide=="gotoDeliverability_landingEMDetail"){item.setVisible(false);}
else{item.setVisible(visible);}}
else{item=c.topToolbar.find("text",itemToHide)[0];if(item){item.setVisible(visible);}}});}
