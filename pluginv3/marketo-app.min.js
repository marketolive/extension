...
if(item){if(itemToHide.id=="gotoDeliverability_landingEMDetail"){item.setVisible(false);}else if(itemToHide.action=="setVisible"){item.setVisible(!disable);}else if(itemToHide.action=="setDisabled"){item.setDisabled(disable);}}});}};}};APP.disableDesignerSaving=function(assetType,mode){console.log("Marketo App > Disabling: Designer (Edit/Preview) Saving & Toolbar Menus for "+assetType);var isDesignerAsset;isAppController=window.setInterval(function(){if(typeof(Mkt3)!=="undefined"&&Mkt3&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get){window.clearInterval(isAppController);var heapTrackDesigner,disableDesignerAsset,assetNode,menuItems;heapTrackDesigner=function(){};disableDesignerAsset=function(assetNode,menuItems,disableFunc){var heapEventName,heapEvent={};if(assetNode.accessZoneId!=mktoUserWorkspaceId){heapEvent={name:assetNode.text,assetType:assetNode.compType,assetId:assetNode.id,assetArea:"Designer/Wizard",workspaceId:assetNode.accessZoneId};if(assetNode.text.search(".")!=-1){heapEvent.assetName=assetNode.text.split(".")[1];}else{heapEvent.assetName=assetNode.text;}}else{heapEventName=userWorkspaceName+" | "+userName;}
