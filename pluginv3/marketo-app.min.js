...
currNode=currNode.parentNode;}}}
return toBeDisabled;break;case"inAppMessage":if(toolbar.getInAppMessage()&&(toolbar.getInAppMessage().get('zoneId')==mktoDefaultWorkspaceId||toolbar.getInAppMessage().get('zoneId')==mktoJapaneseWorkspaceId||toolbar.getInAppMessage().get('zoneId')==mktoFinservWorkspaceId||toolbar.getInAppMessage().get('zoneId')==mktoHealthcareWorkspaceId||toolbar.getInAppMessage().get('zoneId')==mktoHigherEdWorkspaceId)||(toolbar.getInAppMessage().get('zoneId')==mktoMarketingWorkspaceId&&toolbar.getInAppMessage().getNodeJson()&&toolbar.getInAppMessage().getNodeJson().id&&MktExplorer.getNodeById(toolbar.getInAppMessage().getNodeJson().id))){toBeDisabled=true;if(toolbar.getInAppMessage().get('zoneId')==mktoMarketingWorkspaceId){var ii,currNode=MktExplorer.getNodeById(toolbar.getInAppMessage().getNodeJson().id),depth=currNode.getDepth();for(ii=0;ii<depth;ii++){if(currNode.attributes.text==userName){toBeDisabled=false;break;}
currNode=currNode.parentNode;}}}
...
if(Ext4&&Ext4.button&&Ext4.button.Button&&Ext4.button.Button.prototype&&Ext4.button.Button.prototype.showMenu){Ext4.button.Button.prototype.showMenu=function(fromEvent){console.log("Marketo App > Executing: Disable Toolbar buttons & Actions menu for Named Accounts in Account Based Marketing");var mItems=this.menu.items,itemsToDisable=["deleteNamedAccount","newAccountList","renameAccountList","deleteAccountList","mktoCustomObjectPublishBtn","mktoCustomObjectDeleteBtn","mktoCustomObjectFieldDeleteBtn","mktoCustomActivityPublishBtn","mktoCustomActivityDeleteBtn","mktoCustomActivityFieldDeleteBtn",];if(mItems){itemsToDisable.forEach(function(itemToDisable){var item=mItems.get(itemToDisable);if(item){item.setDisabled(true);}});}
mItems=Ext4.ComponentQuery.query("menu [action=delete],"+"menu [action=editTestDevice],"+"menu [action=deleteTestDevice],");if(mItems){mItems.forEach(function(item){if(item){item.setDisabled(true);}});}
...
else{console.log("Marketo App > Skipped: Disable Toolbar buttons & Actions menu for In App Messages in Marketing Activities");}
...
else{console.log("Marketo App > Skipped: Disable Content & Actions menu for a Nurture Program Stream in Marketing Activities");}}
...
if(accountString.search(mktoAccountStringsMatch)!=-1||APP.getCookie("toggleState")=="false"){console.log("Marketo App > Location: MarketoLive Instance");window.mkto_live_plugin_state=true;if(userId.search("^admin\.?[a-z]{0,2}@mktodemoaccount")!=-1||userId.search("^mktodemoaccount[a-z0-9]*@marketo\.com")!=-1||userId.search("^marketodemo.*@gmail\.com$")!=-1){console.log("Marketo App > User: Admin");APP.disableDemoPluginCheck();if(APP.getCookie("toggleState")!="false"){return;}
...
break;case mktoInAppMessageWizardFragment:console.log("Marketo App > Location: In App Message Wizard");if(Ext4&&Ext4.getStore('InAppMessage')&&Ext4.getStore('InAppMessage').load){console.log("Callback for In App Message Editor");Ext4.getStore('InAppMessage').load(loadParameters);}
break;case mktoSocialAppWizardFragment:console.log("Marketo App > Location: Social App Wizard");if(Ext4&&Ext4.getStore('SocialApp')&&Ext4.getStore('SocialApp').load){console.log("Callback for Social App Editor");Ext4.getStore('SocialApp').load(loadParameters);}
break;case mktoABtestWizardFragment:console.log("Marketo App > Location: A/B Test Wizard");var isABtestWizard=window.setInterval(function(){if(Mkt3&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor")&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor()&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record&&(Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoDefaultWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoJapaneseWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoFinservWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoHealthcareWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoHigherEdWorkspaceId)){console.log("Marketo App > Loaded: A/B Test Wizard");window.clearInterval(isABtestWizard);APP.disableSaving();}},0);break;case mktoEmailTestWizardFragment:console.log("Marketo App > Location: Email Test Group Wizard");var isEmailTestWizard=window.setInterval(function(){if(Mkt3&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor")&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor()&&Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record&&(Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoDefaultWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoJapaneseWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoFinservWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoHealthcareWorkspaceId||Mkt3.app.controllers.get("Mkt3.controller.editor.wizard.Editor").getEditor().record.get("zoneId")==mktoHigherEdWorkspaceId)){console.log("Marketo App > Loaded: Email Test Group Wizard");window.clearInterval(isEmailTestWizard);APP.disableSaving();}},0);break;default:break;}}
...
else if(currUrlFragment.search(mktoEmailPreviewFragmentRegex)!=-1){console.log("Marketo App > Location: Email Previewer");}}}},0);}
APP.overrideSuperballMenuItems();}
else if(accountString.search(mktoDemoAccountMatch)!=-1){APP.overrideSuperballMenuItems();if(currUrlFragment==mktoMyMarketoFragment){APP.overrideHomeTiles();}
...
else if(accountString.search(mktoAccountStringAbm)!=-1){APP.disableMenus();APP.disableFormSaveButtons();APP.disableAdminSaveButtons();}}},0);}