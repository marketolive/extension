...
console.log("Marketo App > checkBadExtension Msg > Response: "+JSON.strignify(response));}else{APP.validateDemoExtensionCheck(false);}
console.log("Marketo App > checkBadExtension Msg > Last Error: "+chrome.runtime.lastError);});}else{APP.validateDemoExtensionCheck(false);}
console.log("Marketo App > checkExtensionVersion Msg > Last Error: "+chrome.runtime.lastError);});if(MktPage.userid&&MktPage.userName){var mktoRole=MktPage.userName.match(/\[[^\]]+\]/);if(mktoRole!=null){mktoRole=mktoRole[0].replace(/^\[([^\]]+)]$/,"$1");}
