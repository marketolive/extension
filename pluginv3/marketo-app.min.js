...
isEmailEditor2=window.setInterval(function(){if(action=="edit"){console.log("Marketo App > Overlaying: Email Editor");if(overlay(document.getElementsByTagName("iframe")[0].contentWindow.document.getElementsByTagName("body")[0])==true){console.log("Marketo App > Overlaying: Email Interval is Cleared");window.clearInterval(isEmailEditor2);}}
else if(action=="preview"){console.log("Marketo App > Overlaying: Email Previewer");emailDocuments=document.getElementsByTagName("iframe");for(ii=0;ii<emailDocuments.length;ii++){currEmailDocument=emailDocuments[ii];if(currEmailDocument.getAttribute("id").search("^iframeComponent-")!=-1){numOfIframes++;if(overlay(currEmailDocument.contentWindow.document.getElementsByTagName("body")[0])==true){numOfIframesReplaced++;}}}
if(numOfIframesReplaced==numOfIframes){console.log("Marketo App > Overlaying: Email Interval is Cleared");window.clearInterval(isEmailEditor2);}}},0);}
