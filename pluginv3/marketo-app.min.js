...
switch(itemToHide.id){case"pageEdit_landingLPDetail":var origHandler=item.handler;item.setHandler(function(){if(attr&&attr.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){var discardMsg=Ext.MessageBox.show({title:"MarketoLive",msg:"Discarding Draft",progress:false,wait:false,width:270,closable:true});Mkt.app.DesignStudio.Pages.discardDraft({triggeredFrom:"button",xtra:attr});discardMsg.hide();Ext.MessageBox.hide();window.setTimeout(function(){Mkt.app.DesignStudio.Pages.editPage({triggeredFrom:"button",el:this.getEl()});Ext.MessageBox.hide();},waitAfterDiscard);}else{origHandler.apply(this,arguments);}});break;case"emailEdit_landingEMDetail":var origHandler=item.handler;item.setHandler(function(button,e){if(attr&&attr.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){var discardMsg=Ext.MessageBox.show({title:"MarketoLive",msg:"Discarding Draft",progress:false,wait:false,width:270,closable:true});Mkt.app.DesignStudio.Emails.discardDraft({triggeredFrom:'button',xtra:attr,el:this.getEl()});discardMsg.hide();Ext.MessageBox.hide();window.setTimeout(function(button,e){Mkt.app.DesignStudio.Emails.editDraft({triggeredFrom:'button',panelId:attr.panelId});Ext.MessageBox.hide();},waitAfterDiscard);}else{origHandler.apply(this,arguments);}});break;}}});}};}};APP.hideOtherToolbarItems=function(itemsToHide){var isTopToolbarActive=window.setInterval(function(){console.log("Marketo App > Hiding: Other Toolbar Items");if(typeof(MktCanvas)!=="undefined"&&MktCanvas&&MktCanvas.getActiveTab()&&MktCanvas.getActiveTab().getTopToolbar()){console.log("Marketo App > Executing: Hiding Other Toolbar Items");window.clearInterval(isTopToolbarActive);var topToolbar=MktCanvas.getActiveTab().getTopToolbar();itemsToHide.forEach(function(itemToHide){if(itemToHide.id){item=topToolbar.items.get(itemToHide.id);}else if(itemToHide.text){item=topToolbar.find("text",itemToHide.text)[0];}
