...
APP.heapTrack("track",heapEvent);}},0);};APP.disableDesignerSaving=function(assetType,mode){console.log("Marketo App > Disabling: Designer (Edit/Preview) Saving & Toolbar Menus for "+assetType);var isAppController;isAppController=window.setInterval(function(){if(typeof(Mkt3)!=="undefined"&&Mkt3&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get){window.clearInterval(isAppController);var logo=APP.getCookie("logo"),heroBackground=APP.getCookie("heroBackground"),color=APP.getCookie("color"),isCustomCompany,disableDesignerAsset,assetNode,menuItems;if(logo!=null||color!=null||heroBackground!=null){isCustomCompany=true;}
disableDesignerAsset=function(assetNode,menuItems,disableFunc){console.log("Marketo App > Executing: Disabling Designer (Edit/Preview)");var heapEvent={name:"",assetName:"",assetType:assetNode.compType,assetId:assetNode.id,workspaceId:assetNode.accessZoneId,workspaceName:""};switch(mode){case"edit":heapEvent.assetArea="Editor";break;case"preview":heapEvent.assetArea="Previewer";break;default:heapEvent.assetArea="Designer";break;}
heapEvent.workspaceName=APP.getWorkspaceName(assetNode.accessZoneId);if(assetNode.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1){heapEvent.name=heapEvent.workspaceName;}else if(assetNode.accessZoneId==mktoUserWorkspaceId){heapEvent.name=heapEvent.workspaceName;heapEvent.userFolder=userName;}else{heapEvent.name="User's Workspace"}
