...
return emIds;}
chrome.runtime.onMessage.addListener(function(message,sender,sendResponse){if(message.action=="newCompany"){console.log("Marketo App > Capturing: New Company Logo");switch(message.assetType){case"email":if(message.assetView=="edit"){APP.overlayEmail("edit");}
else if(message.assetView=="preview"){APP.overlayEmail("preview");}
break;case"landingPage":if(message.assetView=="edit"){APP.overlayLandingPageDesigner();}
else if(message.assetView=="preview"){APP.overlayLandingPageDesigner();}
break;default:break;}}});if((currentUrl.search(mktoAppDomain)!=-1||currentUrl.search(mktoDesignerDomain)!=-1||currentUrl.search(mktoWizard)!=-1)){console.log("Marketo App > Location: Marketo URL");var isMktPageApp=window.setInterval(function(){if(typeof(MktPage)!=="undefined"){console.log("Marketo App > Location: Marketo Page");var accountString,userId;if(MktPage.savedState&&MktPage.savedState.custPrefix&&MktPage.userid&&Mkt3&&Mkt3.DL&&Mkt3.DL.getDlToken()){window.clearInterval(isMktPageApp);accountString=MktPage.savedState.custPrefix;userId=MktPage.userid.toLowerCase();currUrlFragment=Mkt3.DL.getDlToken();if(Mkt3.DL.dl&&Mkt3.DL.dl.dlCompCode){currCompFragment=Mkt3.DL.dl.dlCompCode;}
...
window.onhashchange=function(){console.log("Marketo App > Window: Hash Changed");currentUrl=window.location.href;var isNewUrlFragment=window.setInterval(function(){if(Mkt3&&Mkt3.DL&&Mkt3.DL.getDlToken()&&Mkt3.DL.dl&&Mkt3.DL.dl.dlCompCode){if(currUrlFragment!=Mkt3.DL.getDlToken()){window.clearInterval(isNewUrlFragment);currUrlFragment=Mkt3.DL.getDlToken();currCompFragment=Mkt3.DL.dl.dlCompCode;console.log("Marketo App > Loaded: New URL Fragment = "+currUrlFragment);if(currUrlFragment==mktoMyMarketoFragment){APP.overrideHomeTiles();}
else if(currUrlFragment==mktoMarketingActivitiesDefaultFragment||currUrlFragment==mktoMarketingActivitiesMarketingFragment||currUrlFragment==mktoMarketingActivitiesJapaneseFragment||currUrlFragment==mktoMarketingActivitiesFinservFragment||currUrlFragment==mktoMarketingActivitiesHealthcareFragment||currUrlFragment==mktoMarketingActivitiesHigherEdFragment||currUrlFragment==mktoLeadDatabaseDefaultFragment||currUrlFragment==mktoLeadDatabaseMarketingFragment||currUrlFragment==mktoLeadDatabaseJapaneseFragment||currUrlFragment==mktoLeadDatabaseFinservFragment||currUrlFragment==mktoLeadDatabaseHealthcareFragment||currUrlFragment==mktoLeadDatabaseHigherEdFragment){APP.disableButtons();}
else if(currUrlFragment==mktoAnalyticsDefaultFragment){APP.overrideAnalyticsTiles();}
