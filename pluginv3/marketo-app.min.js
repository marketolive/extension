...
if(emailInsightsMenuItem){emailInsightsMenuItem.href=mktoEmailInsightsLink;emailInsightsMenuItem.update();}else{clonedMenuItem=menu.items.items[0].cloneConfig();clonedMenuItem.setText("Email Insights");clonedMenuItem.setIconCls("mki3-email-insights-svg");clonedMenuItem.href=mktoEmailInsightsLink;clonedMenuItem.hrefTarget="_blank";clonedMenuItem.update();menu.add(clonedMenuItem);}
...
var params={compId:this.programId,portletOrdering:Ext.encode(newPortletOrder)};MktSession.ajaxRequest('marketingEvent/orderLocalAssetPortlets',{serializeParms:params,localAssetManager:this,portletOrdering:newPortletOrder,onMySuccess:this.updatePortletOrderSuccess});}else{console.log("Marketo App > Disabling: Updating of Portlet Order");}};}};APP.overrideTreeNodeExpand=function(){console.log("Marketo App > Overriding: Tree Node Expand");if(typeof(MktAsyncTreeNode)!=="undefined"&&MktAsyncTreeNode.prototype&&MktAsyncTreeNode.prototype.expand&&userName){MktAsyncTreeNode.prototype.expand=function(){console.log("Marketo App > Executing: Tree Node Expand");var attr=this.attributes,ii;if(this.text==userWorkspaceName||(this.parentNode.text==userWorkspaceName&&this.attributes.system==true)||(this.parentNode.parentNode!=null&&this.parentNode.parentNode.text==userWorkspaceName&&this.attributes.system==true)){for(ii=0;ii<this.childNodes.length;ii++){if(this.childNodes[ii].attributes.system==false){if(this.childNodes[ii].text.toLowerCase()!==userName){this.childNodes[ii].hidden=true;}else{}}}}
...
return true;}else{return false;}};}};APP.evaluateMenu=function(triggeredFrom,menu,canvas,toolbar){console.log("Marketo App > Evaluating: Menu");if(userName){var toBeDisabled=false;switch(triggeredFrom){case"tree":if(menu&&menu.currNode&&menu.currNode.attributes&&(menu.currNode.attributes.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1||menu.currNode.attributes.accessZoneId==mktoUserWorkspaceId)){toBeDisabled=true;if(menu.currNode.attributes.accessZoneId==mktoUserWorkspaceId){var ii,currNode=menu.currNode,depth=currNode.getDepth();for(ii=0;ii<depth;ii++){if(currNode.attributes.text==userName){toBeDisabled=false;break;}
currNode=currNode.parentNode;}}}else if((!menu||!menu.currNode||!menu.currNode.attributes||!menu.currNode.attributes.accessZoneId)&&(canvas&&canvas.config&&(canvas.config.accessZoneId.toString().search(mktoGoldenWorkspacesMatch)!=-1||(canvas.config.accessZoneId==mktoUserWorkspaceId&&((canvas.config.expNodeId&&MktExplorer.getNodeById(canvas.config.expNodeId))||(canvas.config.dlZoneFolderId&&MktExplorer.getNodeById(canvas.config.dlZoneFolderId))))))){toBeDisabled=true;if(canvas.config.accessZoneId==mktoUserWorkspaceId){var ii,currNode,depth;if(canvas.config.expNodeId){currNode=MktExplorer.getNodeById(canvas.config.expNodeId);}else{currNode=MktExplorer.getNodeById(canvas.config.dlZoneFolderId);}
...
Mkt3.DlManager.setCustomToken(step.getId());tree.expandPath(step.parentNode.getPath());tree.getView().getSelectionModel().select(step);this.updateFlowButtons();editor.fireEvent('stepchange',stepId,previousStepId);Ext4.resumeLayouts(true);if(editor.down("[action=create]").isVisible()){editor.down("[action=create]").setDisabled(true);}};}else{console.log("Marketo App > Skipped: Disable Create button in Wizard Editors");}
...
var mItems=dashboard.query("abmAccountListToolbar [action=removeNamedAccount],");if(mItems){mItems.forEach(function(item){if(item){item.destroy();}});}};}else{console.log("Marketo App > Skipped: Disable Toolbar Buttons for ABM > Account Lists > Named Accounts");}
...
var disable=APP.evaluateMenu("inAppMessage",null,null,this),mItems=Ext4.ComponentQuery.query("inAppMessage contextMenu [action=unapprove],"+"inAppMessage contextMenu [action=approve],"+"inAppMessage contextMenu [action=clone],"+"inAppMessage contextMenu [action=delete],"+"inAppMessage contextMenu [action=sendDraftSample],"+"inAppMessage contextMenu [action=approveDraft],");if(mItems){mItems.forEach(function(item){if(item){item.setDisabled(disable);}});}};}else{console.log("Marketo App > Skipped: Disable Toolbar Buttons & Actions Menu for Marketing Activities > In-App Messages");}
if(typeof(Mkt3)!=="undefined"&&Mkt3&&Mkt3.controller&&Mkt3.controller.smsMessage&&Mkt3.controller.smsMessage.SmsMessage&&Mkt3.controller.smsMessage.SmsMessage.prototype&&Mkt3.controller.smsMessage.SmsMessage.prototype.loadToolbar){var prevSmsMessageToolbar=Mkt3.controller.inAppMessage.InAppMessage.prototype.loadToolbar;Mkt3.controller.smsMessage.SmsMessage.prototype.loadToolbar=function(){console.log("Marketo App > Executing: Disable Toolbar Buttons & Actions Menu for Marketing Activities > SMS Messages");prevSmsMessageToolbar.apply(this,arguments);var toolbar=this.getToolbar(),smsMessage=this.getSmsMessage(),actionsMenu=toolbar.down('.contextMenu'),toolbarComponents=toolbar.query('component')||[],i=0,il=toolbarComponents.length,toolbarComponent,text;actionsMenu.record=smsMessage;for(;i<il;i++){toolbarComponent=toolbarComponents[i];if(Ext4.isDefined(toolbarComponent.iconCls)&&Ext4.isFunction(toolbarComponent.setIconCls)){toolbarComponent.setIconCls(toolbarComponent.iconCls);}
if((Ext4.isDefined(toolbarComponent.text)||Ext4.isFunction(toolbarComponent.getText))&&Ext4.isFunction(toolbarComponent.setText)){text=Ext4.isFunction(toolbarComponent.getText)?toolbarComponent.getText():toolbarComponent.text;toolbarComponent.setText(text);}}
...
if(item){if(itemToHide.id=="gotoDeliverability_landingEMDetail"){item.setVisible(false);}else if(itemToHide.action=="setVisible"){item.setVisible(!disable);}else if(itemToHide.action=="setDisabled"){item.setDisabled(disable);}}});}};}};APP.disableDesignerSaving=function(assetType,mode){console.log("Marketo App > Disabling: Designer (Edit/Preview) Saving & Toolbar Menus for "+assetType);var isDesignerAsset;isAppController=window.setInterval(function(){if(typeof(Mkt3)!=="undefined"&&Mkt3&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get){window.clearInterval(isAppController);var heapTrackDesigner,disableDesignerAsset,assetNode,menuItems;disableDesignerAsset=function(assetNode,menuItems,disableFunc){console.log("Marketo App > Executing: Disabling Designer (Edit/Preview)");var heapEvent={name:assetNode.text,assetType:assetNode.compType,assetId:assetNode.id,assetArea:"Designer/Wizard",workspaceId:assetNode.accessZoneId};if(assetNode.accessZoneId!=mktoUserWorkspaceId){if(assetNode.text.search(".")!=-1){heapEvent.assetName=assetNode.text.split(".")[1];}else{heapEvent.assetName=assetNode.text;}}else{heapEvent.name=userWorkspaceName+" | "+userName;}
...
currPosition=x;}
window.location=pod.valueSet[currPosition].url;});}}}},0);};APP.overrideSaving=function(){console.log("Marketo App > Overriding: Saving for Nurture Streams");if(typeof(Mkt3)!=="undefined"&&Mkt3.data&&Mkt3.data.Store&&Mkt3.data.Store.prototype&&Mkt3.data.Store.prototype.sync){var prevDataStoreSync=Mkt3.data.Store.prototype.sync;Mkt3.data.Store.prototype.sync=function(){console.log("Marketo App > Executing: Override Saving for Nurture Streams (sync)");if(window.location.href.search("\/#"+mktoCalendarFragment)!=-1){Mkt3.data.Store.prototype.sync=prevDataStoreSync;}else{var disable;if(typeof(MktCanvas)!=="undefined"&&MktCanvas.getActiveTab()&&APP.getCookie("toggleState")!="false"){disable=APP.evaluateMenu("button",null,MktCanvas.getActiveTab(),null);}else if(APP.getCookie("toggleState")=="false"){disable=true;}
if(!disable){if(this.autoSyncSuspended){this.autoSync=true;this.autoSyncSuspended=false;}
if(this.getProxy()instanceof Mkt3.data.proxy.AjaxPost){Mkt3.Synchronizer.sync(this);}else{this.callParent(arguments);}}else{console.log("Marketo App > Disabling: Saving for Nurture Streams (sync)");}}};}
if(typeof(Ext4)!=="undefined"&&Ext4.data&&Ext4.data.Model&&Ext4.data.Model.prototype&&Ext4.data.Model.prototype.destroy){Ext4.data.Model.prototype.destroy=function(options){console.log("Marketo App > Executing: Override Saving for Nurture Streams (destroy)");var disable;if(typeof(MktCanvas)!=="undefined"&&MktCanvas.getActiveTab()&&APP.getCookie("toggleState")!="false"){disable=APP.evaluateMenu("button",null,MktCanvas.getActiveTab(),null);}else if(APP.getCookie("toggleState")=="false"){disable=true;}
...
return me;}else{console.log("Marketo App > Disabling: Saving for Nurture Streams (destroy)");}};}};APP.enableSaving=function(){console.log("Marketo App > Enabling: Saving for Editors");if(typeof(Mkt3)!=="undefined"&&Mkt3.data&&Mkt3.data.Store&&Mkt3.data.Store.prototype&&Mkt3.data.Store.prototype.sync){Mkt3.data.Store.prototype.sync=function(){console.log("Marketo App > Executing: Enable Saving for Editors (sync)");if(this.autoSyncSuspended){this.autoSync=true;this.autoSyncSuspended=false;}
...
APP.trackTreeNodeSelection=function(){console.log("Marketo App > Tracking: Tree Node Selection");if(typeof(MktExplorer)!=="undefined"&&MktExplorer.selectTreeNode){MktExplorer.selectTreeNode=function(node){var currNode=node,heapEventName,heapEvent={assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,workspaceId:currNode.attributes.accessZoneId};if(currNode.attributes.accessZoneId!=mktoUserWorkspaceId){heapEventName=currNode.text;for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" | "+heapEventName;}}else{heapEventName=userWorkspaceName+" | "+userName;}
heapEvent.name=heapEventName;heapTrack("track",heapEvent);if(!node||!this.boundTree)
return false;if(!node.isSelected()){this.boundTree.expandPath(node.getPath(),'id');node.render();node.select();return true;}else{return false;}};}};APP.trackOtherAssets=function(){console.log("Marketo App > Tracking: Social App or Push Notification Selection");if(typeof(MktExplorer)!=="undefined"&&MktExplorer.getNodeById&&MktExplorer.getNodeById(currUrlFragment.substring(0,currUrlFragment.length-5))){var node=currNode=MktExplorer.getNodeById(currUrlFragment.substring(0,currUrlFragment.length-5)),heapEventName,heapEvent={assetName:currNode.text,assetId:currNode.attributes.id,assetType:currNode.attributes.compType,workspaceId:currNode.attributes.accessZoneId};if(currNode.attributes.accessZoneId!=mktoUserWorkspaceId){heapEventName=currNode.text;for(var ii=0;ii<node.getDepth()-1;ii++){currNode=currNode.parentNode;heapEventName=currNode.text+" | "+heapEventName;}}else{heapEventName=userWorkspaceName+" | "+userName;}
heapEvent.name=heapEventName;heapTrack("track",heapEvent);}};var heapTrack=function(action,event){var isHeapAnalytics=window.setInterval(function(){if(typeof(heap)!=="undefined"){window.clearInterval(isHeapAnalytics);switch(action){case"id":var oneLoginEmail=APP.getCookie("onelogin_email"),oneLoginFirstName=APP.getCookie("onelogin_first_name"),oneLoginLastName=APP.getCookie("onelogin_last_name");if(oneLoginEmail){heap.identify(oneLoginEmail);}else if(MktPage&&MktPage.userid){heap.identify(MktPage.userid);}
if(oneLoginFirstName&&oneLoginLastName){console.log("Marketo App > Heap Analytics ID: "+oneLoginFirstName+" "+oneLoginLastName);heap.addUserProperties({Name:oneLoginFirstName+" "+oneLoginLastName});}else if(MktPage&&MktPage.userName){console.log("Marketo App > Heap Analytics ID: "+MktPage.userName);heap.addUserProperties({Name:MktPage.userName});}
...
if(event.assetArea){heapArea=event.assetArea;}else if(MktPage&&MktPage.baseTitle){heapArea=MktPage.baseTitle.split("•")[0].trimRight();}else{heapArea="Unknown";}
if(event){heapEventProps={app:heapApp,asset:event.assetName,assetId:event.assetId,assetType:event.assetType,workspaceId:event.workspaceId,area:heapArea,url:currentUrl};console.log("Marketo App > Tracking: Heap Event: "+event.name+"\n"+JSON.stringify(heapEventProps,null,2));heap.track(event.name,heapEventProps);}else{var heapEventTitle=document.title.replace(" - "+document.location.protocol+"//"+document.location.host+"/",""),heapAsset,heapArea;if(heapEventTitle.search("•")!=-1){heapAsset=heapEventTitle.split("|")[1].trimLeft().split("•")[0].trimRight();heapArea=heapEventTitle.split("|")[1].trimLeft().split("•")[1].trimLeft();}else if(heapEventTitle.search("|")!=-1){heapAsset="Home";heapArea=heapEventTitle.split("|")[1].trimLeft();}else{heapAsset="Home";heapArea=heapEventTitle;}
...
if(userId.search("^admin\.?[a-z]{0,2}@mktodemoaccount")!=-1||userId.search("^mktodemoaccount[a-z0-9]*@marketo\.com")!=-1||userId.search("^marketodemo.*@gmail\.com$")!=-1){console.log("Marketo App > User: Admin");APP.disableDemoPluginCheck();if(APP.getCookie("toggleState")!="false"){return;}else{console.log("Marketo App > User: Admin is now a normal user");}}
APP.disableDemoPluginCheck();if(currUrlFragment==mktoMyMarketoFragment){APP.overrideHomeTiles();}else if(currUrlFragment.search(mktoDisableButtonsFragmentMatch)!=-1){APP.disableButtons();}else if(currUrlFragment==mktoAnalyticsDefaultFragment){APP.overrideAnalyticsTiles();}else if(currUrlFragment.search(mktoOtherAssetsFragmentMatch)!=-1){APP.trackOtherAssets();}else if(currUrlFragment.search(mktoAnalyzersFragmentMatch)!=-1){console.log("Marketo App > Location: Analytics");APP.injectAnalyzerNavBar();}else if(currUrlFragment==mktoAdBridgeSmartListFragment){console.log("Marketo App > Location: Ad Bridge Smart List");APP.openAdBridgeModal();}
if(!currCompFragment||currCompFragment.search(mktoDesignersFragmentMatch)==-1){if(accountString.search(mktoAccountStringsMatch)!=-1){APP.discardDrafts(accountString);APP.overrideTreeNodeExpand();APP.overrideTreeNodeCollapse();APP.overrideSaving();APP.disableDragAndDrop();APP.disableMenus();APP.hideToolbarItems();APP.disableFormSaveButtons();APP.disableAdminSaveButtons();APP.overrideSmartCampaignSaving();APP.trackTreeNodeSelection();APP.overrideUpdatePortletOrder();APP.overrideNewProgramCreate();APP.overrideAssetSaveEdit();APP.overrideNewAssetCreate();APP.overrideNewFolders();APP.overrideRenamingFolders();APP.hideFoldersOnImport();APP.disableConfirmationMessage();}else{APP.overrideSaving();APP.overrideSmartCampaignSaving();APP.overrideUpdatePortletOrder();APP.disableConfirmationMessage();}
if(accountString==mktoAccountStringQe){APP.disableMenus();APP.hideToolbarItems();APP.disableFormSaveButtons();APP.disableAdminSaveButtons();}}else{console.log("Marketo App > Location: Designers/Wizards");switch(currCompFragment){case mktoLandingPageEditFragment:console.log("Marketo App > Location: Landing Page Editor");APP.disableDesignerSaving("landingPage","edit");break;case mktoLandingPagePreviewFragment:console.log("Marketo App > Location: Landing Page Previewer");APP.disableDesignerSaving("landingPage","preview");break;case mktoLandingPagePreviewDraftFragment:console.log("Marketo App > Location: Landing Page Draft Previewer");APP.disableDesignerSaving("landingPage","preview");break;case mktoEmailEditFragment:if(currUrlFragment.search(mktoEmailPreviewFragmentRegex)==-1){console.log("Marketo App > Location: Email Editor");APP.disableDesignerSaving("email","edit");}else{console.log("Marketo App > Location: Email Previewer");APP.disableDesignerSaving("email","preview");}
