...
else if(this.id=="leadDbListMenu"||this.id=="segmentationMenu"){disable=APP.evaluateMenu("tree",attr,this,canvas,null);}
else{disable=APP.evaluateMenu(this.triggeredFrom,attr,this,canvas,null);}
itemsToDisable.forEach(function(itemToDisable){var item=mItems.get(itemToDisable);if(item){item.setDisabled(disable);}});if(this.ownerCt&&this.ownerCt.text.search("^View:")!=-1){var ii;for(ii=0;ii<this.items.items.length;ii++){switch(this.items.items[ii].text){case"Create View":this.items.items[ii].setDisabled(disable);break;case"Edit Default":this.items.items[ii].setDisabled(disable);break;default:break;}}}
