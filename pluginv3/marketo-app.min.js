...
APP.evaluateMoveItem=function(nodeToMove,destNode){console.log("Marketo App > Evaluating: Move Item");var ii,userName,currNode,depth,fromUserFolder=false,toUserFolder=false,userId=MktPage.userid.toLowerCase();if(userId.search("\.demo@marketo.com$")!=-1){userName=userId.split(".demo")[0];}
else{userName=userId.split("@")[0];}
currNode=nodeToMove;depth=currNode.getDepth();for(ii=0;ii<depth;ii++){if(currNode.text==userName){fromUserFolder=true;break;}
currNode=currNode.parentNode;}
if(fromUserFolder){currNode=destNode;depth=currNode.getDepth();for(ii=0;ii<depth;ii++){if(currNode.text==userName){toUserFolder=true;return true;break;}
currNode=currNode.parentNode;}}
else{return false;}}
