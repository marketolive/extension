...
else if(currCompFragment==mktoLandingPageDesignerFragment||currCompFragment==mktoLandingPagePreviewFragment){console.log("Marketo App > Location: Landing Page Designer");var isLandingPageDesigner=window.setInterval(function(){if(typeof(Mkt3)!=="undefined"&&Mkt3.app&&Mkt3.app.controllers&&Mkt3.app.controllers.get("Mkt3.controller.editor.LandingPage")&&Mkt3.app.controllers.get("Mkt3.controller.editor.LandingPage").getLandingPage()){console.log("Marketo App > Loaded: Landing Page Designer");window.clearInterval(isLandingPageDesigner);if(Mkt3.app.controllers.get("Mkt3.controller.editor.LandingPage").getLandingPage().getNodeJson()){var assetNode=Mkt3.app.controllers.get("Mkt3.controller.editor.LandingPage").getLandingPage().getNodeJson(),heapEvent={name:assetNode.text,assetType:assetNode.compType,assetId:assetNode.id,assetArea:"Designer/Wizard",workspaceId:assetNode.accessZoneId};if(assetNode.text.search(".")!=-1){heapEvent.assetName=assetNode.text.split(".")[1];}
else{heapEvent.assetName=assetNode.text;}
heapTrack("track",heapEvent);}
if(Mkt3.app.controllers.get("Mkt3.controller.editor.LandingPage").getLandingPage().get("zoneId").toString().search(mktoGoldenWorkspacesMatch)!=-1){APP.disablePropertyPanelSaving();if(typeof(Ext4)!=="undefined"&&Ext4.ComponentQuery&&Ext4.ComponentQuery.query){var mItems=Ext4.ComponentQuery.query("lpEditor menu [action=approveAndClose],"+"lpEditor menu [action=disableMobileVersion],"+"lpEditor menu [action=uploadImage],"+"lpEditor menu [action=grabImages],");if(mItems){console.log("Marketo App > Disabling Landing Page Designer Toolbar Menus");mItems.forEach(function(item){if(item){item.setDisabled(true);}});}}}}},0);return;}
else{console.log("Marketo App > Location: Designers/Wizards");var loadParameters={filters:[{property:'id',value:Mkt3.DL.dl.compId}],callback:function(records){records.forEach(function(record){if(record.getNodeJson()){var assetNode=record.getNodeJson(),heapEvent={name:assetNode.text,assetType:assetNode.compType,assetId:assetNode.id,assetArea:"Designer/Wizard",workspaceId:assetNode.accessZoneId};if(assetNode.text.search(".")!=-1){heapEvent.assetName=assetNode.text.split(".")[1];}
else{heapEvent.assetName=assetNode.text;}
heapTrack("track",heapEvent);}
if(record.get('zoneId')){console.log("Marketo App > currAssetWorkspaceId = "+currAssetWorkspaceId);var currAssetWorkspaceId=record.get('zoneId');if(currAssetWorkspaceId.toString().search(mktoGoldenWorkspacesMatch)!=-1||APP.getCookie("toggleState")=="false"){APP.disableSaving();if(typeof(Ext4)!=="undefined"&&Ext4.ComponentQuery&&Ext4.ComponentQuery.query){var mItems=Ext4.ComponentQuery.query("emailEditor2 menu [action=approveEmail],"+"emailEditor2 menu [action=sendTestEmail],"+"emailEditor2 menu [action=uploadImage],"+"emailEditor2 menu [action=grabImages],"+"emailEditor2 menu [action=saveAsTemplate],"+"email2EditorPreviewToolbar [action=sendSampleEmail],"+"menu [action=approveEmail],"+"menu [action=sendSampleEmail],"+"inAppMessageEditor menu [action=approveAndClose],");if(mItems){console.log("Marketo App > Disabling Designer Toolbar Menus");mItems.forEach(function(item){if(item){item.setDisabled(true);}});}}}}});}};switch(currCompFragment){case mktoEmailDesignerFragment:if(currUrlFragment.search(mktoEmailPreviewFragmentRegex)==-1){console.log("Marketo App > Location: Email Previewer");APP.disableDesignerToolbarMenus();}
else{console.log("Marketo App > Location: Email Editor");}
if(typeof(Ext4)!=="undefined"&&Ext4.getStore('Email')&&Ext4.getStore('Email').load){console.log("Marketo App > Callback: Email Editor");Ext4.getStore('Email').load(loadParameters);}
