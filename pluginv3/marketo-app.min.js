...
APP.overlayEmail=function(action){console.log("Marketo App > Overlaying: Email");var isEmailEditor2,clearOverlayVars,overlay,ii,isMktoImgReplaced=false,isMktoTextReplaced=false,isMktoSubTextReplaced=false,isMktoButtonReplaced=false,isMktoEmail1Replaced=false,dayNames=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],monthNames=["JAN","FEB","MAR","APR","MAY","JUNE","JULY","AUG","SEPT","OCT","NOV","DEC"],date=new Date(),dayOfWeek=dayNames[date.getDay()],month=monthNames[date.getMonth()],dayOfMonth,year=date.getFullYear(),logoMktoNameRegex=new RegExp("logo","i"),mainTitleMktoNameRegex=new RegExp("main title","i"),subTitleMktoNameRegex=new RegExp("subtitle","i"),buttonTextRegex=new RegExp("signup|sign up|call to action|cta","i"),logo=APP.getCookie("logo"),color=APP.getCookie("color"),company,companyName,emailIframes;switch(date.getDate()){case 1:dayOfMonth="1st";break;case 2:dayOfMonth="2nd";break;case 3:dayOfMonth="3rd";break;default:dayOfMonth=date.getDate()+"th";break;}
...
overlay=function(emailDocument){if(emailDocument!=null){var emailBody=emailDocument.getElementsByTagName("body")[0],mktoImgs=emailBody.getElementsByClassName("mktoImg"),mktoTexts=emailBody.getElementsByClassName("mktoText"),mktoButtons=emailBody.getElementsByClassName("secondary-font button"),logoSwapCompany=emailDocument.getElementById("logo-swap-company"),logoSwapContainer=emailDocument.getElementById("logo-swap-container"),logoSwapCompanyContainer=emailDocument.getElementById("logo-swap-company-container"),logoBkg=emailDocument.getElementById("logo-bkg"),buttonBkg=emailDocument.getElementById("button-bkg");if(mktoImgs&&!isMktoImgReplaced){var currMktoImg,currMktoImgMktoName,currMktoImgTag;for(ii=0;ii<mktoImgs.length;ii++){currMktoImg=mktoImgs[ii];currMktoImgMktoName=currMktoImg.getAttribute("mktoname");if(currMktoImgMktoName&&currMktoImgMktoName.search(logoMktoNameRegex)!=-1){currMktoImgTags=currMktoImg.getElementsByTagName("img")[0];if(currMktoImgTag&&currMktoImgTag.src){console.log("Marketo App > Overlaying: Email 2.0 Company Logo");currMktoImgTag.setAttribute("src",logo);isMktoImgReplaced=true;break;}}}}
if(mktoTexts&&!isMktoSubTextReplaced&&!isMktoTextReplaced){var currMktoText,currMktoTextMktoName;for(ii=0;ii<mktoTexts.length;ii++){currMktoText=mktoTexts[ii];currMktoTextMktoName=currMktoText.getAttribute("mktoname");if(currMktoTextMktoName&&currMktoTextMktoName.search(mainTitleMktoNameRegex)!=-1){if(currMktoText.innerHTML){console.log("Marketo App > Overlaying: Email 2.0 Company Name");currMktoText.innerHTML=companyName+" Invites You<br><br>PREMIER BUSINESS EVENT<br>OF THE YEAR";isMktoTextReplaced=true;}}
...
else if(action=="preview"){console.log("Marketo App > Overlaying: Email Previewer");emailIframes=document.getElementsByTagName("iframe");if(overlay(emailIframes[2].contentWindow.document)&&overlay(emailIframes[3].contentWindow.document)){console.log("Marketo App > Overlaying: Email Interval is Cleared");window.clearInterval(isEmailEditor2);}}},0);}
