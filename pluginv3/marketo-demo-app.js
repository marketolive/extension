...
APP.overrideSuperballMenuItems = function () {
  ...
  if (typeof MktPage !== 'undefined' && MktPage.showSuperMenu) {
    MktPage.showSuperMenu = function () {
      ...
      if (!menu) {
        ...
        if (menu && menu.items && menu.items.items) {
          ...
          var ii, currSuperBallMenuItem, performanceInsightsMenuItem, emailInsightsMenuItem, deliverabilityToolsMenuItem, clonedMenuItem
    ...
    if (
      ...
      if (
        ...
        var container = MktCanvas.getActiveTab().el.dom.childNodes[0].childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[0],
          ...
          performanceInsightsTileExists = (emailInsightsTileExists = false)
        ...
        if (!emailInsightsTileExists) {
          var emailInsightsTileOuterHTML =
              '<div class="x4-btn mkt3-analyticsTile mkt3-analyticsHomeTile x4-btn-default-small x4-icon-text-left x4-btn-icon-text-left x4-btn-default-small-icon-text-left" id="analyticsTile-1059"><em id="analyticsTile-1059-btnWrap"><a id="analyticsTile-1059-btnEl" href="' +
              mktoEmailInsightsLink +
              '" class="x4-btn-center" target="_blank" role="link" style="height: 160px;"><span id="analyticsTile-1059-btnInnerEl" class="x4-btn-inner">Email Insights</span><span id="analyticsTile-1059-btnIconEl" class="x4-btn-icon mki3-email-insights-svg"></span></a></em></div>',
...
APP.cloneProgram = function (cloneToSuffix, cloneToFolderId, origProgramTreeNode) {
  var newProgramName, newProgramType, result
...
APP.cloneNurtureCadence = function (origProgramCompId, newProgramCompId) {
  var getNurtureCadence, getOrigNurtureCadenceResponse, getNewNurtureCadenceResponse
...
APP.cloneSmartCampaignState = function (origProgramCompId, newProgramCompId, forceActivate) {
  var getOrigProgramAssetDetailsResponse, getNewProgramAssetDetailsResponse
  ...
  if (getOrigProgramAssetDetailsResponse && getNewProgramAssetDetailsResponse) {
    ...
    setSmartCampaignState = function (getOrigProgramAssetDetailsResponse, getNewProgramAssetDetailsResponse) {
      var currOrigProgramSmartCampaign, currNewProgramSmartCampaign, getScheduleResponse
    ...
    if (getOrigProgramAssetDetailsResponse.assetList[0].tree.length == getNewProgramAssetDetailsResponse.assetList[0].tree.length) {
      var currOrigProgramAsset, currNewProgramAsset
      ...
      for (var ii = 0; ii < getOrigProgramAssetDetailsResponse.assetList[0].tree.length; ii++) {
        ...
        if (currOrigProgramAsset.navType == 'MA' && currNewProgramAsset.navType == 'MA') {
          setSmartCampaignState(
            APP.getProgramAssetDetails(currOrigProgramAsset.compId),
            APP.getProgramAssetDetails(currNewProgramAsset.compId)
          )
        }
      }
    }
  ...
  applyProgramReportFilter = function (getNewProgramAssetDetailsResponse, cloneToFolderId) {
    ...
    for (var ii = 0; ii < getNewProgramAssetDetailsResponse.assetList[0].tree.length; ii++) {
      ...
      if (currNewReport.compType == 'Report') {
        var reportFilterType, selectedNodes
...
APP.applyMassClone = function (forceReload) {
                              ...
                              if (currTreeNode.children[jj].compType == 'Marketing Folder') {
                                ...
                                if (cloneFolderResponse) {
                                  ...
                                  for (var kk = 0; currFolderTreeNode.children && kk < currFolderTreeNode.children.length; kk++) {
                                    ...
                                    cloneProgramResponse = APP.cloneProgram(
                                      cloneToSuffix,
                                    ...
                                    if (cloneProgramResponse) {
                                      ...
                                      if (cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compType == 'Nurture Program') {
                                        APP.cloneNurtureCadence(
                                          currOrigProgramTreeNode.compId,
                                          cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId
                                        )
                                      }

                                      getNewProgramAssetDetailsResponse = APP.cloneSmartCampaignState(
                                        currOrigProgramTreeNode.compId,
                                        cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId,
                                        scForceActivate
                                      )
  ...
  if (typeof Ext !== 'undefined' && Ext && Ext.menu && Ext.menu.Menu && Ext.menu.Menu.prototype && Ext.menu.Menu.prototype.showAt) {
        ...
        if (!isMktoImgReplaced && logo && (mktoLogo1 || mktoLogo2 || mktoImgs.length != 0)) {
          if (mktoLogo1 || mktoLogo2) {
            ...
            if (mktoLogo1 && mktoLogo1.getAttribute('display') != 'none') {
              console.log('Marketo Demo App > Overlaying: Landing Page Company Logo 1')
            ...
            if (mktoLogo2 && mktoLogo2.getAttribute('display') != 'none') {
              console.log('Marketo Demo App > Overlaying: Landing Page Company Logo 2')
      ...
      if (logoImg && textBackground && textBackground.style && bannerBackground && bannerBackground.style && mainTitle && subTitle) {
        ...
        if (logo) {
          ...
          logoImg.onload = function () {
            var logoHeightsRatio, logoWidth, logoNewWidth, logoNewHeight, logoStyle
            ...
            console.log(
              'Marketo Demo App > Overlaying: Original Landing Page Company Logo Dimensions = ' + logoNewWidth + ' x ' + logoNewHeight
            )
          }
        }
      ...
      if (emailBody && emailBody.innerHTML) {
        ...
        if (!isMktoImgReplaced && logo && (mktoLogo1 || mktoLogo2 || mktoImgs.length != 0)) {
          if (mktoLogo1 || mktoLogo2) {
            ...
            if (mktoLogo1 && mktoLogo1.getAttribute('display') != 'none') {
              console.log('Marketo Demo App > Overlaying: Email 2.0 Company Logo 1')
              mktoLogo1.style.width = 'auto'
              mktoLogo1.style.height = 'auto'
              mktoLogo1.setAttribute('src', logo)
              isMktoImgReplaced = true
            ...
            if (mktoLogo2 && mktoLogo2.getAttribute('display') != 'none') {
              console.log('Marketo Demo App > Overlaying: Email 2.0 Company Logo 2')
              mktoLogo2.style.width = 'auto'
              mktoLogo2.style.height = 'auto'
              mktoLogo2.setAttribute('src', logo)
              isMktoImgReplaced = true
    ...
    if (typeof heap !== 'undefined' && heap) {
      ...
      switch (action) {
        ...
        case 'id':
          ...
          if (MktPage && MktPage.savedState && MktPage.savedState.custPrefix) {
            heap.addUserProperties({
        ...
        case 'track':
          var heapApp, heapArea, heapEventProps
...
var isMktPageDemoApp = window.setInterval(function () {
  if (typeof MktPage !== 'undefined') {
    ...
    var currUrlFragment, currCompFragment

    if (typeof Mkt3 !== 'undefined' && Mkt3 && Mkt3.DL && Mkt3.DL.getDlToken() && Mkt3.DL.getDlToken() != '') {
      window.clearInterval(isMktPageDemoApp)

      currUrlFragment = Mkt3.DL.getDlToken()

      if (Mkt3.DL.dl && Mkt3.DL.dl.dlCompCode) {
    ...
    if (
      ...
      //TODO put the MF hash hunter
      APP.applyMassClone()
    } else if (currCompFragment && currCompFragment.search(mktoDesignersFragmentMatch) != -1 && currUrlFragment.search(/[0-9]+$/) != -1) {
    ...
    window.onhashchange = function () {
      var isNewUrlDemoFragment = window.setInterval(function () {
        if (typeof Mkt3 !== 'undefined' && Mkt3 && Mkt3.DL && Mkt3.DL.getDlToken()) {
