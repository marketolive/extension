var mktoMyMarketoFragment="MM0A1",mktoEmailInsightsLink="http://www.marketolive.com/en/analytics/email-insights-summit-demo-1",mktoEmailDeliverabilityToolsLink="https://250ok.com/login",mktoEmailEditFragment="EME",mktoEmailPreviewFragmentRegex=new RegExp("^EME[0-9]+&isPreview","i"),mktoEmailPreviewFragment2="EME[0-9]+&isPreview",mktoEmailPreviewFragment="EMP",mktoLandingPageEditFragment="LPE",mktoLandingPagePreviewFragment="LPP",mktoLandingPagePreviewDraftFragment="LPPD",mktoPushNotificationEditFragment="MPNE",mktoMobilePushNotificationPreviewFragment="MPNP",mktoInAppMessageEditFragment="IAME",mktoInAppMessagePreviewFragment="IAMP",mktoDesignersFragmentMatch="^"+mktoEmailEditFragment+"$|^"+mktoEmailPreviewFragment2+"|^"+mktoEmailPreviewFragment+"$|^"+mktoLandingPageEditFragment+"$|^"+mktoLandingPagePreviewFragment+"$|^"+mktoLandingPagePreviewDraftFragment+"$",APP=APP||{};APP.getCookie=function(cookieName){console.log("Marketo App > Getting: Cookie "+cookieName);var name=cookieName+'=',cookies=document.cookie.split(';'),currCookie;for(var ii=0;ii<cookies.length;ii++){currCookie=cookies[ii].trim();if(currCookie.indexOf(name)==0){return currCookie.substring(name.length,currCookie.length);}}
...
editAssetVars=function(asset){waitForLoadMsg.show();var assetVars=asset.getResponsiveVarValues(),newTitle=asset.componentsStore.getAt(asset.componentsStore.findExact("name","title")).fragmentsStore.data.items[0],newSubtitle=asset.componentsStore.getAt(asset.componentsStore.findExact("name","subtitle")).fragmentsStore.data.items[0];newTitle.data.content=newTitle.data.body="<div>"+title+"</div>";newSubtitle.data.content=newSubtitle.data.body=subtitle;Mkt3.controller.editor.predefinedLayoutLandingPage.LPEditorPredefinedLayout.prototype.updateRichTextComponent.call(Mkt3.app.controllers.get("Mkt3.controller.editor.predefinedLayoutLandingPage.LPEditorPredefinedLayout"),newTitle);Mkt3.controller.editor.predefinedLayoutLandingPage.LPEditorPredefinedLayout.prototype.updateRichTextComponent.call(Mkt3.app.controllers.get("Mkt3.controller.editor.predefinedLayoutLandingPage.LPEditorPredefinedLayout"),newSubtitle);asset.setResponsiveVarValue(headerBgColor,color);asset.setResponsiveVarValue(headerLogoImg,logo);asset.setResponsiveVarValue(heroBgImg,heroBackground);asset.setResponsiveVarValue(formButtonBgColor,color);asset.setResponsiveVarValue(footerLogoImg,logo);for(var ii=0;ii<Object.keys(assetVars).length;ii++){var currVariableKey=Object.keys(assetVars)[ii],currVariableValue=Object.values(assetVars)[ii].toString();if(currVariableValue==null){currVariableValue="";}
...
if(Mkt3.DL.dl&&Mkt3.DL.dl.dlCompCode){currCompFragment=Mkt3.DL.dl.dlCompCode;if(currCompFragment.search(mktoDesignersFragmentMatch)!=-1){console.log("Marketo Demo App > Location: Designers/Wizards");switch(currCompFragment){case mktoLandingPageEditFragment:console.log("Marketo Demo App > Location: Landing Page Editor");APP.editAssetVariables("landingPage","edit");break;case mktoLandingPagePreviewFragment:console.log("Marketo Demo App > Location: Landing Page Previewer");APP.editAssetVariables("landingPage","preview");break;case mktoLandingPagePreviewDraftFragment:console.log("Marketo Demo App > Location: Landing Page Draft Previewer");APP.editAssetVariables("landingPage","preview");break;case mktoEmailEditFragment:if(currUrlFragment.search(mktoEmailPreviewFragmentRegex)==-1){console.log("Marketo Demo App > Location: Email Editor");APP.editAssetVariables("email","edit");}else{console.log("Marketo Demo App > Location: Email Previewer");APP.editAssetVariables("email","preview");}
break;}}}}}},0);};}},0);