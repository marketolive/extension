...
massCloneForm.buttons[1].setHandler(function(){var waitMsg=new Ext.Window({closable:true,modal:true,width:520,height:225,cls:'mktModalForm',title:'Please Wait',html:'<b><ins>Mass Cloning:  '+massCloneForm.currNode.text+' ...</ins></b><br><br>This may take several minutes depending on the quantity of programs and assets contained therein.'}),cloneToFolderId=massCloneForm.find("fieldLabel","Clone To")[0].getValue(),cloneToAffix=massCloneForm.find("fieldLabel","Program Affix")[0].getValue(),cloneToTreeNode=MktExplorer.getNodeById(cloneToFolderId),_this=this,waitMsgShow,currTreeNode;massCloneForm.close();waitMsgShow=waitMsg.show();var isWaitMsgShow=window.setInterval(function(){if(waitMsgShow){window.clearInterval(isWaitMsgShow);for(var ii=0;ii<_this.currNode.attributes.children.length;ii++){currTreeNode=_this.currNode.attributes.children[ii];if(currTreeNode.compType=="Marketing Folder"){var newFolderName,createFolderResponse,cloneProgramResponse,getOrigProgramSettingsResponse,getNewProgramSettingsResponse,getOrigNurtureCadenceResponse,getNewNurtureCadenceResponse,getOrigLocalAssetDetailsResponse,getNewLocalAssetDetailsResponse,getScheduleResponse,currOrigProgramTreeNode;if(currTreeNode.text.search(/\([^\)]*\)$/)!=-1){newFolderName=currTreeNode.text.replace(/\([^\)]*\)$/,"("+cloneToAffix+")");}else{newFolderName=currTreeNode.text+" ("+cloneToAffix+")";}
APP.webRequest('/explorer/createProgramFolder','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&text='+newFolderName+'&parentId='+cloneToFolderId+'&tempNodeId=ext-'+cloneToFolderId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);createFolderResponse=JSON.parse(response);});if(createFolderResponse.JSONResults.appvars.createProgramFolderResult=="success"){for(var jj=0;jj<currTreeNode.children.length;jj++){var newProgramName,newProgramType;currOrigProgramTreeNode=currTreeNode.children[jj];if(currOrigProgramTreeNode.text.search(/\([^\)]*\)$/)!=-1){newProgramName=currOrigProgramTreeNode.text.replace(/\([^\)]*\)$/,"("+cloneToAffix+")");}else{newProgramName=currOrigProgramTreeNode.text+" ("+cloneToAffix+")";}
