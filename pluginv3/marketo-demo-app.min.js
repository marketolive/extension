...
APP.webRequest('/analytics/applyComponentFilter','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&nodeIds='+selectedNodes+'&filterType='+reportFilterType+'&reportId='+currNewReport.compId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);});}}}};if(getOrigProgramAssetDetailsResponse&&getNewProgramAssetDetailsResponse){applyProgramReportFilter(getOrigProgramAssetDetailsResponse,getNewProgramAssetDetailsResponse);}else if(getOrigProgramAssetDetailsResponse&&newProgramCompId){applyProgramReportFilter(getOrigProgramAssetDetailsResponse,APP.getProgramAssetDetails(newProgramCompId));}else if(origProgramCompId&&getNewProgramAssetDetailsResponse){applyProgramReportFilter(APP.getProgramAssetDetails(origProgramCompId),getNewProgramAssetDetailsResponse);}else if(origProgramCompId&&newProgramCompId){applyProgramReportFilter(APP.getProgramAssetDetails(origProgramCompId),APP.getProgramAssetDetails(newProgramCompId));}};APP.getTags=function(){var result=APP.webRequest('/marketingEvent/getAllDescriptors','&start=0'+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){;console.log(response);response=JSON.parse(response);if(response.success){var currTag,jj=0,customTags=[];for(var ii=0;ii<response.data.descriptors.length;ii++){currTag=response.data.descriptors[ii];if(currTag.type!="channel"){customTags[jj]=currTag;jj++;}}
...
getNewProgramAssetDetailsResponse=APP.cloneSmartCampaignState(currOrigProgramTreeNode.compId,cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId,scForceActivate);APP.setProgramReportFilter(null,getNewProgramAssetDetailsResponse,currOrigProgramTreeNode.compId);}}}}}
