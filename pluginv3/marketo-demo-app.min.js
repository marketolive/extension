...
return true;};APP.applyMassClone=function(){console.log("Marketo Demo App > Applying: Mass Clone Menu Item");massClone=function(){if(this.triggeredFrom=="tree"&&this.get("newLocalAsset")){var massCloneItem=this.get("newLocalAsset").cloneConfig(),massCloneItemId="cloneVertical",currExpNode=MktExplorer.getNodeById(this.currNode.attributes.id);if(!this.get(massCloneItemId)){massCloneItem.itemId=massCloneItemId;massCloneItem.text="Mass Clone";massCloneItem.setHandler(function(el){var cloneFromField=new Mkt.apps.marketingEvent.MarketingEventForm({cloneFromId:this.ownerCt.currNode.attributes.compId,cloneName:this.ownerCt.currNode.attributes.text,accessZoneId:this.ownerCt.currNode.attributes.accessZoneId}).find("fieldLabel","Clone From")[0].cloneConfig(),massCloneForm=new Mkt.apps.marketingEvent.MarketingEventForm({currNode:this.ownerCt.currNode});cloneNoteField=massCloneForm.items.last().hidden,el.parentMenu.hide(true);var isCloneVerticalForm=window.setInterval(function(){if(massCloneForm&&massCloneForm.buttons[1]&&massCloneForm.buttons[1].setHandler&&massCloneForm.find("fieldLabel","Channel")[0]&&massCloneForm.find("fieldLabel","Channel")[0].destroy&&massCloneForm.find("fieldLabel","Description")[0]&&massCloneForm.find("fieldLabel","Description")[0].destroy&&massCloneForm.find("fieldLabel","Program Type")[0]&&massCloneForm.find("fieldLabel","Program Type")[0].destroy&&massCloneForm.find("fieldLabel","Campaign Folder")[0]&&massCloneForm.find("fieldLabel","Campaign Folder")[0].fieldLabel&&massCloneForm.find("fieldLabel","Name")[0]&&massCloneForm.find("fieldLabel","Name")[0].fieldLabel&&massCloneForm.items.last().setText&&massCloneForm.items.last().setVisible&&massCloneForm.setWidth&&massCloneForm.setHeight){window.clearInterval(isCloneVerticalForm);massCloneForm.setWidth(520);massCloneForm.setHeight(450);massCloneForm.setTitle("Mass Clone");massCloneForm.buttons[1].setText("Clone");massCloneForm.buttons[1].currNode=massCloneForm.currNode;massCloneForm.find("fieldLabel","Program Type")[0].destroy();massCloneForm.find("fieldLabel","Description")[0].destroy();massCloneForm.find("fieldLabel","Channel")[0].destroy();massCloneForm.find("fieldLabel","Campaign Folder")[0].fieldLabel="Clone To";massCloneForm.find("fieldLabel","Name")[0].fieldLabel="Program Affix";if(cloneFromField){massCloneForm.insert(0,cloneFromField);}
massCloneForm.buttons[1].setHandler(function(){var waitMsg=new Ext.Window({closable:true,modal:true,width:520,height:225,cls:'mktModalForm',title:'Please Wait',html:'<b><ins>Mass Cloning the '+massCloneForm.currNode.text+' folder ...</ins></b><br><br>This may take several minutes depending on the quantity of programs and assets contained therein.'}),cloneToFolderId=massCloneForm.find("fieldLabel","Clone To")[0].getValue(),cloneToVerticalName=massCloneForm.find("fieldLabel","Program Affix")[0].getValue(),cloneToTreeNode=MktExplorer.getNodeById(cloneToFolderId),_this=this,waitMsgShow,currTreeNode;massCloneForm.close();waitMsgShow=waitMsg.show();var isWaitMsgShow=window.setInterval(function(){if(waitMsgShow){window.clearInterval(isWaitMsgShow);for(var ii=0;ii<_this.currNode.attributes.children.length;ii++){currTreeNode=_this.currNode.attributes.children[ii];if(currTreeNode.compType=="Marketing Folder"){var newFolderName=currTreeNode.text.replace(/\([^\)]*\)$/,"("+cloneToVerticalName+")"),createFolderResponse,cloneProgramResponse,getOrigProgramSettingsResponse,getNewProgramSettingsResponse,getOrigNurtureCadenceResponse,getNewNurtureCadenceResponse,getOrigLocalAssetDetailsResponse,getNewLocalAssetDetailsResponse,getScheduleResponse,currOrigProgramTreeNode;APP.webRequest('/explorer/createProgramFolder','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&text='+newFolderName+'&parentId='+cloneToFolderId+'&tempNodeId=ext-'+cloneToFolderId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);createFolderResponse=JSON.parse(response);});if(createFolderResponse.JSONResults.appvars.createProgramFolderResult=="success"){for(var jj=0;jj<currTreeNode.children.length;jj++){var newProgramName,newProgramType;currOrigProgramTreeNode=currTreeNode.children[jj];if(currOrigProgramTreeNode.text.search(/\([^\)]*\)$/)!=-1){newProgramName=currOrigProgramTreeNode.text.replace(/\([^\)]*\)$/,"("+cloneToVerticalName+")");}else{newProgramName=currOrigProgramTreeNode.text+" ("+cloneToVerticalName+")";}
