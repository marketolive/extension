...
return true;};APP.applyMassClone=function(){console.log("Marketo Demo App > Applying: Mass Clone Menu Item");massClone=function(){if(this.triggeredFrom=="tree"&&this.get("newLocalAsset")){var massCloneItem=this.get("newLocalAsset").cloneConfig(),massCloneItemId="cloneVertical",currExpNode=MktExplorer.getNodeById(this.currNode.attributes.id);massCloneItem.itemId=massCloneItemId;massCloneItem.text="Mass Clone";massCloneItem.setHandler(function(el){var cloneFromField=new Mkt.apps.marketingEvent.MarketingEventForm({cloneFromId:this.ownerCt.currNode.attributes.compId,cloneName:this.ownerCt.currNode.attributes.text,accessZoneId:this.ownerCt.currNode.attributes.accessZoneId}).find("fieldLabel","Clone From")[0].cloneConfig(),massCloneForm=new Mkt.apps.marketingEvent.MarketingEventForm({currNode:this.ownerCt.currNode});cloneNoteField=massCloneForm.items.last().hidden,el.parentMenu.hide(true);var isCloneVerticalForm=window.setInterval(function(){if(massCloneForm&&massCloneForm.buttons[1]&&massCloneForm.buttons[1].setHandler&&massCloneForm.find("fieldLabel","Channel")[0]&&massCloneForm.find("fieldLabel","Channel")[0].destroy&&massCloneForm.find("fieldLabel","Description")[0]&&massCloneForm.find("fieldLabel","Description")[0].destroy&&massCloneForm.find("fieldLabel","Program Type")[0]&&massCloneForm.find("fieldLabel","Program Type")[0].destroy&&massCloneForm.find("fieldLabel","Campaign Folder")[0]&&massCloneForm.find("fieldLabel","Campaign Folder")[0].fieldLabel&&massCloneForm.find("fieldLabel","Name")[0]&&massCloneForm.find("fieldLabel","Name")[0].fieldLabel&&massCloneForm.items.last().setText&&massCloneForm.items.last().setVisible){window.clearInterval(isCloneVerticalForm);massCloneForm.setTitle("Mass Clone");massCloneForm.buttons[1].setText("Clone");massCloneForm.buttons[1].currNode=massCloneForm.currNode;massCloneForm.find("fieldLabel","Program Type")[0].destroy();massCloneForm.find("fieldLabel","Description")[0].destroy();massCloneForm.find("fieldLabel","Channel")[0].destroy();massCloneForm.find("fieldLabel","Campaign Folder")[0].fieldLabel="Clone To";massCloneForm.find("fieldLabel","Name")[0].fieldLabel="Program Affix";if(cloneFromField){massCloneForm.insert(0,cloneFromField);}
massCloneForm.buttons[1].setHandler(function(){var waitMsg=new Ext.Window({closable:true,modal:false,width:350,height:150,cls:'mktModalForm',title:'Please Wait',html:'Mass Cloning '+massCloneForm.currNode.text+'Folder ...'}),cloneToFolderId=massCloneForm.find("fieldLabel","Clone To")[0].getValue(),cloneToVerticalName=massCloneForm.find("fieldLabel","Program Affix")[0].getValue(),cloneToTreeNode=MktExplorer.getNodeById(cloneToFolderId),_this=this,waitMsgShow,currTreeNode;massCloneForm.close();waitMsgShow=waitMsg.show();var isWaitMsgShow=window.setInterval(function(){if(waitMsgShow){window.clearInterval(isWaitMsgShow);for(var ii=0;ii<_this.currNode.attributes.children.length;ii++){currTreeNode=_this.currNode.attributes.children[ii];if(currTreeNode.compType=="Marketing Folder"){var newFolderName=currTreeNode.text.replace(/\([^\)]*\)$/,"("+cloneToVerticalName+")"),createFolderResponse,cloneProgramResponse,getOrigProgramSettingsResponse,getNewProgramSettingsResponse,getOrigNurtureCadenceResponse,getNewNurtureCadenceResponse,getOrigLocalAssetDetailsResponse,getNewLocalAssetDetailsResponse,getScheduleResponse,currOrigProgramTreeNode;webRequest('/explorer/createProgramFolder','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&text='+newFolderName+'&parentId='+cloneToFolderId+'&tempNodeId=ext-'+cloneToFolderId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);createFolderResponse=JSON.parse(response);});if(createFolderResponse.JSONResults.appvars.createProgramFolderResult=="success"){for(var jj=0;jj<currTreeNode.children.length;jj++){var newProgramName,newProgramType;currOrigProgramTreeNode=currTreeNode.children[jj];if(currOrigProgramTreeNode.text.search(/\([^\)]*\)$/)!=-1){newProgramName=currOrigProgramTreeNode.text.replace(/\([^\)]*\)$/,"("+cloneToVerticalName+")");}else{newProgramName=currOrigProgramTreeNode.text+" ("+cloneToVerticalName+")";}
switch(currOrigProgramTreeNode.compType){case"Marketing Program":newProgramType="program";break;case"Nurture Program":newProgramType="nurture";break;case"Marketing Event":newProgramType="event";break;case"Email Batch Program":newProgramType="emailBatchProgram";break;case"In-App Program":newProgramType="inAppProgram";break;default:newProgramType="program";break;}
webRequest('/marketingEvent/createMarketingProgramSubmit','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&name='+newProgramName+'&description='+'&parentFolderId='+createFolderResponse.JSONResults.actions[0].parameters[0][0].id+'&cloneFromId='+currOrigProgramTreeNode.compId+'&type='+newProgramType+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);cloneProgramResponse=JSON.parse(response);});if(cloneProgramResponse.JSONResults.appvars.result=="Success"){webRequest('/marketingEvent/getProgramSettingsData','&start=0'+'&query='+'&compId='+currOrigProgramTreeNode.compId+'&compType='+currOrigProgramTreeNode.compType+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);getOrigProgramSettingsResponse=JSON.parse(response);});if(getOrigProgramSettingsResponse.success){var currYear=new Date().getFullYear(),currMonth=new Date().getMonth()+1,costAmount;for(var kk=0;kk<24;kk++){var costDate;if(currMonth>12){currMonth=1;currYear+=1;}
costDate=currYear.toString()+"-"+currMonth.toString();currMonth+=1;costAmount=parseInt(getOrigProgramSettingsResponse.data[0].summaryData.amount)+Math.floor(Math.random()*100);webRequest('/marketingEvent/setCostSubmit','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&compId='+cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId+'&costId='+'&type=period'+'&startDate='+costDate+'&amount='+costAmount.toString()+'&description='+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);});}}else{console.log("Get Program Settings Failed");}
webRequest('/marketingEvent/getProgramSettingsData','&start=0'+'&query='+'&compId='+cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId+'&compType='+cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compType+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);getNewProgramSettingsResponse=JSON.parse(response);});if(getNewProgramSettingsResponse.success){var currSetting,verticalSettingValue,verticalSettingId,verticalSettingDescId,verticalTagData;for(var i=0;i<getNewProgramSettingsResponse.data.length;i++){currSetting=getNewProgramSettingsResponse.data[i];if(currSetting.summaryData.name=="Vertical"){verticalSettingValue=MktExplorer.getNodeById(cloneToFolderId).text;verticalSettingId=parseInt(currSetting.id.replace(/^PD-/,''));verticalSettingDescId=currSetting.descriptorId;verticalTagData=encodeURIComponent('{"programId":'+cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId+',"programDescriptorId":'+verticalSettingId+',"descriptorId":'+verticalSettingDescId+',"descriptorValue":"'+verticalSettingValue+'"}');break;}}
if(verticalTagData){webRequest('/marketingEvent/setProgramDescriptorSubmit','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&compId='+cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId+'&_json='+verticalTagData+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);});}}
if(cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compType=="Nurture Program"){var origProgramFilter=encodeURIComponent('[{"property":"id","value":'+currOrigProgramTreeNode.compId+'}]'),newProgramFilter=encodeURIComponent('[{"property":"id","value":'+cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId+'}]'),fields=encodeURIComponent('["+tracks"]');webRequest('/data/nurture/retrieve','filter='+origProgramFilter+'&fields='+fields+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);getOrigNurtureCadenceResponse=JSON.parse(response);});webRequest('/data/nurture/retrieve','filter='+newProgramFilter+'&fields='+fields+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);getNewNurtureCadenceResponse=JSON.parse(response);});if(getOrigNurtureCadenceResponse.success&&getNewNurtureCadenceResponse.success&&getOrigNurtureCadenceResponse.data[0].tracks.length==getNewNurtureCadenceResponse.data[0].tracks.length){var currOrigStream,currNewStream,streamCadences='[';for(var i=0;i<getOrigNurtureCadenceResponse.data[0].tracks.length;i++){currOrigStream=getOrigNurtureCadenceResponse.data[0].tracks[i];currNewStream=getNewNurtureCadenceResponse.data[0].tracks[i];if(i!=0){streamCadences+=',';}
...
streamCadences+=']';streamCadences=streamCadences.replace(/\"null\"/g,'null');webRequest('/data/nurtureTrack/update','data='+encodeURIComponent(streamCadences)+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);getNurtureCadenceResponse=JSON.parse(response);});}}
webRequest('/marketingEvent/getLocalAssetDetails','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&compId='+currOrigProgramTreeNode.compId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);getOrigLocalAssetDetailsResponse=JSON.parse(response);});webRequest('/marketingEvent/getLocalAssetDetails','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&compId='+cloneProgramResponse.JSONResults.actions[0].parameters[0][0].compId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);getNewLocalAssetDetailsResponse=JSON.parse(response);});if(getNewLocalAssetDetailsResponse&&getNewLocalAssetDetailsResponse.JSONResults&&getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo){if(getOrigLocalAssetDetailsResponse&&getOrigLocalAssetDetailsResponse.JSONResults&&getOrigLocalAssetDetailsResponse.JSONResults.localAssetInfo&&getOrigLocalAssetDetailsResponse.JSONResults.localAssetInfo.smartCampaigns&&getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.smartCampaigns&&getOrigLocalAssetDetailsResponse.JSONResults.localAssetInfo.smartCampaigns.length==getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.smartCampaigns.length){var currOrigProgramSmartCampaign,currNewProgramSmartCampaign;for(var i=0;i<getOrigLocalAssetDetailsResponse.JSONResults.localAssetInfo.smartCampaigns.length;i++){currOrigProgramSmartCampaign=getOrigLocalAssetDetailsResponse.JSONResults.localAssetInfo.smartCampaigns[i];currNewProgramSmartCampaign=getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.smartCampaigns[i];if(currOrigProgramSmartCampaign.compType==currNewProgramSmartCampaign.compType&&currOrigProgramSmartCampaign.compType=="Smart Campaign"&&currOrigProgramSmartCampaign.name==currNewProgramSmartCampaign.name){if(currOrigProgramSmartCampaign.status==7){webRequest('/smartcampaigns/toggleActiveStatus','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&smartCampaignId='+currNewProgramSmartCampaign.compId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(result){console.log(result);});}else if(currOrigProgramSmartCampaign.status==3||currOrigProgramSmartCampaign.status==5){webRequest('/smartcampaigns/editScheduleRS','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&isRequest=1'+'&smartCampaignId='+currOrigProgramSmartCampaign.compId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);if(response.match(/MktPage\.appVars\.scheduleData = {([^=]|\n|\\n)*}/)[0]){getScheduleResponse=JSON.parse(response.match(/MktPage\.appVars\.scheduleData = {([^=]|\n|\\n)*}/)[0].replace(/MktPage\.appVars\.scheduleData = {/,'{').replace(/\'/g,'"').replace(/\\n */g,'"').replace(/: +/g,'": ').replace(/\"\/\/[^\"]+\"/g,'"').replace(/\"}$/,'}'));}});if(getScheduleResponse){var startAtDate=new Date(Date.parse(getScheduleResponse.start_at)),startAt=startAtDate.getFullYear()+"-"+parseInt(startAtDate.getMonth()+1)+"-"+startAtDate.getDate()+" "+startAtDate.getHours()+":"+startAtDate.getMinutes()+":"+startAtDate.getSeconds();webRequest('/smartcampaigns/recurCampSchedule','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&smartCampaignId='+currNewProgramSmartCampaign.compId+'&recurrence_type='+getScheduleResponse.recurrence_type+'&every_n_unit='+getScheduleResponse.every_n_unit+'&start_at='+startAt+'&end_at='+'&every_weekday='+getScheduleResponse.every_weekday+'&week_mask='+getScheduleResponse.week_mask+'&recurDay_of_month='+getScheduleResponse.recurDay_of_month+'&recurMonth_day_type='+getScheduleResponse.recurMonth_day_type+'&recurMonth_week_type='+getScheduleResponse.recurMonth_week_type+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(result){console.log(result);});}}}}}
if(getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.assetList&&getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.assetList[0]&&getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.assetList[0].tree&&getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.assetList[0].tree.length>0){var currNewReport;for(var i=0;i<getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.assetList[0].tree.length;i++){currNewReport=getNewLocalAssetDetailsResponse.JSONResults.localAssetInfo.assetList[0].tree[i];if(currNewReport.compType=="Report"){var reportId=currNewReport.compId,currNewReportTreeNode=MktExplorer.getNodeById(currNewReport.navType+currNewReport.compId),reportFilterType,selectedNodes;switch(currNewReportTreeNode.attributes.domain){case"EmailPerformance":reportFilterType="maEmail";selectedNodes='["'+cloneToFolderId+'"]';break;case"EngagementTrackPerformance":reportFilterType="nurtureprogram";selectedNodes='["'+cloneToFolderId+'"]';break;case"LandingPagePerformance":reportFilterType="maLanding";selectedNodes='["'+cloneToFolderId+'"]';break;case"ProgramPerformance":reportFilterType="program";selectedNodes='["'+cloneToFolderId+'"]';break;case"LeadPerformance":break;case"CompanyWebActivity":break;case"WebPageActivity":break;}
webRequest('/analytics/applyComponentFilter','ajaxHandler=MktSession&mktReqUid='+new Date().getTime()+Ext.id(null,':')+'&nodeIds='+selectedNodes+'&filterType='+reportFilterType+'&reportId='+reportId+'&xsrfId='+MktSecurity.getXsrfId(),'POST',false,"",function(response){console.log(response);});}}}}}else{console.log("Clone Program Failed");}}}else{console.log("Create Folder Failed");}}}
APP.reloadMarketingActivites();waitMsg.close();}},0);});massCloneForm.show();massCloneForm.items.last().setText("Programs that have a folder depth greater than 1 will not be cloned.");massCloneForm.items.last().setVisible(true);}},0);});if(this.get(massCloneItemId)){if(this.currNode.attributes.compType=="Marketing Folder"&&!this.currNode.attributes.marketingProgramId&&currExpNode&&currExpNode.isExpandable()){this.get(massCloneItemId).destroy();this.addItem(massCloneItem);}else{this.get(massCloneItemId).destroy();}}else if(this.currNode.attributes.compType=="Marketing Folder"&&!this.currNode.attributes.marketingProgramId&&currExpNode&&currExpNode.isExpandable()){this.addItem(massCloneItem);}}};if(typeof(Ext)!=="undefined"&&Ext&&Ext.menu&&Ext.menu.Menu&&Ext.menu.Menu.prototype&&Ext.menu.Menu.prototype.showAt){console.log("Marketo Demo App > Executing: Applying Mass Clone Menu Item");var origFunc=Ext.menu.Menu.prototype.showAt;Ext.menu.Menu.prototype.showAt=function(xy,parentMenu){massClone.apply(this,arguments);origFunc.apply(this,arguments);};}else{console.log("Marketo Demo App > Skipping: Applying Mass Clone Menu Item");}}
APP.getHumanDate=function(){console.log("Marketo Demo App > Getting: Today's Date");var dayNames=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],monthNames=["JAN","FEB","MAR","APR","MAY","JUNE","JULY","AUG","SEPT","OCT","NOV","DEC"],date=new Date(),dayOfWeek=dayNames[date.getDay()],month=monthNames[date.getMonth()],dayOfMonth,year=date.getFullYear();switch(date.getDate()){case 1:dayOfMonth="1st";break;case 2:dayOfMonth="2nd";break;case 3:dayOfMonth="3rd";break;default:dayOfMonth=date.getDate()+"th";break;}
...
return false;};isLandingPageEditor=window.setInterval(function(){if(action=="edit"){console.log("Marketo Demo App > Overlaying: Landing Page Designer");if(document.getElementsByTagName("iframe")[0]&&document.getElementsByTagName("iframe")[0].contentWindow&&document.getElementsByTagName("iframe")[0].contentWindow.document&&document.getElementsByTagName("iframe")[0].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[0].contentWindow.document)||desktopRepeatReadyCount>=maxRepeatReady){console.log("Marketo Demo App > Overlayed: Landing Page Desktop Designer = "+desktopRepeatReadyCount);isDesktopReplaced=true;clearOverlayVars();}else if(desktopPrevReady){desktopRepeatReadyCount++;}else{desktopRepeatReadyCount=1;}
desktopPrevReady=true;}else{desktopPrevReady=false;}
if(isMktoFreeForm&&!isPhoneReplaced&&document.getElementsByTagName("iframe")[1]&&document.getElementsByTagName("iframe")[1].contentWindow&&document.getElementsByTagName("iframe")[1].contentWindow.document&&document.getElementsByTagName("iframe")[1].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[1].contentWindow.document)||phoneRepeatReadyCount>=maxRepeatReady){console.log("Marketo Demo App > Overlayed: Freeform Landing Page Phone Designer = "+phoneRepeatReadyCount);isPhoneReplaced=true;clearOverlayVars();}else if(phonePrevReady){phoneRepeatReadyCount++;}else{phoneRepeatReadyCount=1;}
phonePrevReady=true;}else{phonePrevReady=false;}
if((!isMktoFreeForm&&isDesktopReplaced&&!document.getElementsByTagName("iframe")[1].contentWindow.document.getElementsByTagName("body")[0].innerHTML)||(isMktoFreeForm&&isPhoneReplaced&&isDesktopReplaced)){console.log("Marketo Demo App > Overlaying: Landing Page Interval is Cleared");window.clearInterval(isLandingPageEditor);clearOverlayVars();return true;}}else if(action=="preview"){console.log("Marketo Demo App > Overlaying: Landing Page Previewer");if(!isDesktopReplaced&&document.getElementsByTagName("iframe")[2]&&document.getElementsByTagName("iframe")[2].contentWindow&&document.getElementsByTagName("iframe")[2].contentWindow.document&&document.getElementsByTagName("iframe")[2].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[2].contentWindow.document)||desktopRepeatReadyCount>=maxRepeatReady){console.log("Marketo Demo App > Overlayed: Landing Page Desktop Preview = "+desktopRepeatReadyCount);isDesktopReplaced=true;clearOverlayVars();}else if(desktopPrevReady){desktopRepeatReadyCount++;}else{desktopRepeatReadyCount=1;}
desktopPrevReady=true;}else{desktopPrevReady=false;}
if(!isPhoneReplaced&&document.getElementsByTagName("iframe")[3]&&document.getElementsByTagName("iframe")[3].contentWindow&&document.getElementsByTagName("iframe")[3].contentWindow.document&&document.getElementsByTagName("iframe")[3].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[3].contentWindow.document)||phoneRepeatReadyCount>=maxOtherRepeatReady){console.log("Marketo Demo App > Overlayed: Landing Page Phone Preview = "+phoneRepeatReadyCount);isPhoneReplaced=true;clearOverlayVars();}else if(phonePrevReady){phoneRepeatReadyCount++;}else{phoneRepeatReadyCount=1;}
phonePrevReady=true;}else{phonePrevReady=false;}
if(!isSideBySideDesktopReplaced&&document.getElementsByTagName("iframe")[0]&&document.getElementsByTagName("iframe")[0].contentWindow&&document.getElementsByTagName("iframe")[0].contentWindow.document&&document.getElementsByTagName("iframe")[0].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[0].contentWindow.document)||sideBySideDesktopRepeatReadyCount>=maxOtherRepeatReady){console.log("Marketo Demo App > Overlayed: Landing Page Side by Side Desktop Preview = "+sideBySideDesktopRepeatReadyCount);isSideBySideDesktopReplaced=true;clearOverlayVars();}else if(sideBySideDesktopPrevReady){sideBySideDesktopRepeatReadyCount++;}else{sideBySideDesktopRepeatReadyCount=1;}
sideBySideDesktopPrevReady=true;}else{sideBySideDesktopPrevReady=false;}
if(!isSideBySidePhoneReplaced&&document.getElementsByTagName("iframe")[1]&&document.getElementsByTagName("iframe")[1].contentWindow&&document.getElementsByTagName("iframe")[1].contentWindow.document&&document.getElementsByTagName("iframe")[1].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[1].contentWindow.document)||sideBySidePhoneRepeatReadyCount>=maxOtherRepeatReady){console.log("Marketo Demo App > Overlayed: Landing Page Side by Side Phone Preview = "+sideBySidePhoneRepeatReadyCount);isSideBySidePhoneReplaced=true;clearOverlayVars();}else if(sideBySidePhonePrevReady){sideBySidePhoneRepeatReadyCount++;}else{sideBySidePhoneRepeatReadyCount=1;}
sideBySidePhonePrevReady=true;}else{sideBySidePhonePrevReady=false;}
if(isSideBySidePhoneReplaced&&isSideBySideDesktopReplaced&&isPhoneReplaced&&isDesktopReplaced){console.log("Marketo Demo App > Overlaying: Landing Page Interval is Cleared");window.clearInterval(isLandingPageEditor);clearOverlayVars();return true;}}},0);};APP.overlayEmail=function(action){console.log("Marketo Demo App > Overlaying: Email");var isEmailEditor2,clearOverlayVars,overlay,isMktoHeaderBgColorReplaced=isMktoImgReplaced=isMktoHeroBgReplaced=isMktoTextReplaced=isMktoSubTextReplaced=isMktoButtonReplaced=isMktoEmail1Replaced=editorPrevReady=desktopPrevReady=phonePrevReady=isDesktopPreviewReplaced=isPhonePreviewReplaced=false,logoMktoNameRegex=new RegExp("logo","i"),mainTitleMktoNameRegex=new RegExp("^main title$|^mainTitle$|^main-title$|^hero title$|^heroTitle$|^hero-title$|^title$","i"),subTitleMktoNameRegex=new RegExp("^subtitle$|^sub-title$|^hero subtitle$|^heroSubtitle$|^hero-subtitle$","i"),buttonTextRegex=new RegExp("signup|sign up|call to action|cta|register|more|contribute","i"),saveEditsToggle=APP.getCookie("saveEditsToggleState"),logo=APP.getCookie("logo"),heroBackground=APP.getCookie("heroBackground"),color=APP.getCookie("color"),defaultColor="rgb(42, 83, 112)",logoMaxHeight="55",mktoMainText="You<br>PREMIER BUSINESS EVENT<br>OF THE YEAR",mktoSubText=APP.getHumanDate(),company,companyName,editorRepeatReadyCount=desktopRepeatReadyCount=phoneRepeatReadyCount=0,maxRepeatReady=2000,maxPreviewRepeatReady=3000;if(saveEditsToggle=="true"||(logo==null&&heroBackground==null&&color==null)){return false;}
if(logo!=null){company=logo.split("https://logo.clearbit.com/")[1].split(".")[0];companyName=company.charAt(0).toUpperCase()+company.slice(1);mktoMainText=companyName+" Invites "+mktoMainText;}else{mktoMainText="We Invite "+mktoMainText;}
...
if(!isMktoImgReplaced&&logo&&(mktoLogo||mktoImgs.length!=0)){if(mktoLogo){console.log("Marketo Demo App > Overlaying: Email 2.0 Company Logo for Demo Svcs Template");if(mktoLogo.style.height){mktoLogo.style.setProperty("max-height",mktoLogo.style.height);console.log("Marketo Demo App > Overlaying: Email 2.0 Company Logo Max Height = "+mktoLogo.style.height);}else{mktoLogo.style.setProperty("max-height",mktoLogo.height+"px");console.log("Marketo Demo App > Overlaying: Email 2.0 Company Logo Max Height = "+mktoLogo.height);}
...
if(currMktoTextMktoName&&currMktoTextMktoName.search(mainTitleMktoNameRegex)!=-1){if(currMktoText.innerHTML){console.log("Marketo Demo App > Overlaying: Email 2.0 Company Name in Title");currMktoText.innerHTML=mktoMainText;isMktoTextReplaced=true;}}else if(currMktoTextMktoName&&currMktoTextMktoName.search(subTitleMktoNameRegex)!=-1){if(currMktoText.innerHTML){console.log("Marketo Demo App > Overlaying: Email 2.0 Company Today's Date in Subtitle");currMktoText.innerHTML=mktoSubText;isMktoSubTextReplaced=true;}}
if(isMktoSubTextReplaced&&isMktoTextReplaced){break;}}}}
...
isMktoButtonReplaced=true;}else{for(var ii=0;ii<mktoButtons.length;ii++){var currMktoButton=mktoButtons[ii];if(currMktoButton.innerHTML&&currMktoButton.innerHTML.search(buttonTextRegex)!=-1){if(currMktoButton.style&&currMktoButton.style.backgroundColor){console.log("Marketo Demo App > Overlaying: Email 2.0 Button Company Color");currMktoButton.style.backgroundColor=color;if(currMktoButton.style.getPropertyValue("border")&&currMktoButton.style.getPropertyValue("border")!="none"){currMktoButton.style.setProperty("border","1px solid "+color);}
isMktoButtonReplaced=true;break;}}}}}}
if(logoSwapCompanyContainer&&logoSwapContainer&&logoSwapCompany&&logoBkg){console.log("Marketo Demo App > Overlaying: Email 1.0 Company Logo & Color");if(color){logoBkg.style.backgroundColor=color;}
...
return false;};isEmailEditor2=window.setInterval(function(){if(action=="edit"){console.log("Marketo Demo App > Overlaying: Email Designer");if(document.getElementsByTagName("iframe")[0]&&document.getElementsByTagName("iframe")[0].contentWindow&&document.getElementsByTagName("iframe")[0].contentWindow.document&&document.getElementsByTagName("iframe")[0].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[0].contentWindow.document)||editorRepeatReadyCount>=maxRepeatReady){console.log("Marketo Demo App > Overlayed: Email Designer = "+editorRepeatReadyCount);console.log("Marketo Demo App > Overlaying: Email Interval is Cleared");window.clearInterval(isEmailEditor2);clearOverlayVars();return true;}else if(editorPrevReady){editorRepeatReadyCount++;}else{editorRepeatReadyCount=1;}
editorPrevReady=true;}else{editorPrevReady=false;}}else if(action=="preview"){console.log("Marketo Demo App > Overlaying: Email Previewer");if(!isDesktopPreviewReplaced&&document.getElementsByTagName("iframe")[2]&&document.getElementsByTagName("iframe")[2].contentWindow&&document.getElementsByTagName("iframe")[2].contentWindow.document&&document.getElementsByTagName("iframe")[2].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[2].contentWindow.document)||desktopRepeatReadyCount>=maxPreviewRepeatReady){console.log("Marketo Demo App > Overlayed: Email Desktop Preview = "+desktopRepeatReadyCount);isDesktopPreviewReplaced=true;clearOverlayVars();}else if(desktopPrevReady){desktopRepeatReadyCount++;}else{desktopRepeatReadyCount=1;}
desktopPrevReady=true;}else{desktopPrevReady=false;}
if(!isPhonePreviewReplaced&&document.getElementsByTagName("iframe")[3]&&document.getElementsByTagName("iframe")[3].contentWindow&&document.getElementsByTagName("iframe")[3].contentWindow.document&&document.getElementsByTagName("iframe")[3].contentWindow.document.readyState=="complete"){if(overlay(document.getElementsByTagName("iframe")[3].contentWindow.document)||phoneRepeatReadyCount>=maxPreviewRepeatReady){console.log("Marketo Demo App > Overlayed: Email Phone Preview = "+phoneRepeatReadyCount);isPhonePreviewReplaced=true;clearOverlayVars();}else if(phonePrevReady){phoneRepeatReadyCount++;}else{phoneRepeatReadyCount=1;}
phonePrevReady=true;}else{phonePrevReady=false;}
