...
var URL_PATH = 'm3-dev',
  ...
  analyzerFragmentsMatch =
    mktoAppDomain +
    '/(' +
    oppInfluenceAnalyzerFragment +
    '|' +
    programAnalyzerFragment +
    '|' +
    modelerFragment +
    '|' +
    modelerPreviewFragment +
    '|' +
    successPathAnalyzerFragment +
    ')',
  APP = APP || {}
...
window.onload = function () {
  if (currentUrl.search(mktoAppDomain) != -1 && window.location.pathname != mktoLoginPathName && currentUrl.search(mktoLoginDomain) == -1) {
    ...
    currentUrl.search(mktoEmailInsightsDomain) != -1 ||
