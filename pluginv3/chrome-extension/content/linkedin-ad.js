...
APP.insertAd = function (ad) {
  ...
  try {
    ...
    for (let post of posts) {
      if (
        post.className == 'relative ember-view' &&
    ...
    if (topAd) {
      var actorPicture = topAd.querySelector('a[data-control-name="actor_picture"]'),
        ...
        image =
          topAd.querySelector('a[class="tap-target app-aware-link ember-view"]') ||
          topAd.querySelector('a[class="feed-shared-image__image-link app-aware-link ember-view"]'), //feed-shared-image__image-link app-aware-link ember-view
      ...
      if (actor) {
        ...
        else actor.firstElementChild.insertAdjacentHTML = '<span class="feed-shared-actor__description Sans-13px-black-55%">Promoted</span>'
      }
