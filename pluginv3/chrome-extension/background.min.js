...
function reloadCompany(){console.log("Background > Reloading: Company Logo & Color");var queryInfo={"currentWindow":true,"url":mktoDesignerMatchPattern},message={"action":"newCompany","assetType":"","assetView":""};chrome.tabs.query(queryInfo,function(tabs){var ii,currTab;for(ii=0;ii<tabs.length;ii++){currTab=tabs[ii];if(currTab.url.search("#"+mktoEmailDesignerFragment+"[0-9]+$")!=-1){message.assetType="email";message.assetView="edit";}
...
chrome.tabs.sendMessage(tabs[ii].id,message,function(response){console.log("Background > Receiving: Message Response from Marketo App: "+response);});}});}
