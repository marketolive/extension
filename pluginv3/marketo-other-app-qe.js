...
var URL_PATH = 'm3-dev',
  ...
  mktoAccountString106 = 'srcempty_case1',
  mktoAccountString106d = 'srcempty_case1',
  mktoLiveMasterFriendlyName = 'QE MarketoLive Master',
  mktoLive106FriendlyName = 'QE MarketoLive',
  mktoLive106dFriendlyName = 'QE MarketoLive for Partners',
...
APP.waitForHeap = function (callback) {
  var isHeapAnalytics = window.setInterval(function () {
    if (typeof heap !== 'undefined' && heap && heap.loaded && heap.identify && heap.addUserProperties && heap.addEventProperties) {
    ...
    case mktoLiveRtpHostname.test(window.location.hostname):
      switch (true) {
        ...
        case mktoWebPersonalizationPathname.test(window.location.pathname):
          if (document.getElementsByClassName('page-title').length > 0 && document.getElementsByClassName('page-title')[0].innerHTML) {
            APP.heapEventProps({
