...
if (current_pod.indexOf() == -1) {
	deleteCookie('userPod');	
}
var cust_prefix = getCookie('custPrefix');
//console.log(cust_prefix);
if (cust_prefix.indexOf() == -1) {
...
function MarketoAgile() {
	...
	else if (cust_prefix == 'mktodemoaccount106a') {
		//Change all use case links to redirects to Marketo use cases
		var demo_hrefs_agile = new Array();
		demo_hrefs_agile[0] = 'https://'+current_pod+'.marketo.com/#PG4217A1'; // DIY Design
		demo_hrefs_agile[1] = 'https://'+current_pod+'.marketo.com/#SC14945A1'; // Power Automation
	...
	else if (cust_prefix == 'mktodemoaccount106b') {
		//Change all use case links to redirects to Marketo use cases
		var demo_hrefs_agile = new Array();
		demo_hrefs_agile[0] = 'https://'+current_pod+'.marketo.com/#PG4217A1'; // DIY Design
		demo_hrefs_agile[1] = 'https://'+current_pod+'.marketo.com/#SC14945A1'; // Power Automation
...
if (domain == 'marketolive.com') {
	...
	$(document).ready(function() {
		...
		window.addEventListener('message', function(event) {
			if (event.data.type && event.data.type == 'custPrefix') {
				console.log(event.data.text);
				detectedPrefix = event.data.text;
				console.log('PAGE (ContentScript): Receive Demo Script Message = ' + detectedPrefix);
				setCookie('custPrefix', detectedPrefix, 365, 'marketolive.com', true);
			}
		}, false);
		window.addEventListener('message', function(event) {
			if (event.data.type && event.data.type == 'userPod') {
				console.log(event.data.text);
				detectedPod = event.data.text;
				console.log('PAGE (ContentScript): Receive Demo Script Message = ' + detectedPod);
				setCookie('userPod', detectedPod, 365, 'marketolive.com', true);
			}
		}, false);
		/*
		if (detectedPrefix !== 'undefined' && detectedPod !== 'undefined') {
			window.postMessage({
				type : 'PageMsg',
				text : 'mkto_app_info'
			}, 'https://marketolive.com/');
		...
		*/
		...
		if (window.location.href == 'https://marketolive.com/m2/go-agile/business.html#reload') {
