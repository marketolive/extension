...
var supported_pods = ['app-sjp', 'app-ab07', 'app-ab08'];
var supported_prefixes = ['mktodemoaccount106', 'mktodemoaccount106a', 'mktodemoaccount106b'];
...
//Get Marketo App Parameters
function getMktoAppParms() {
	function getPrefix() {
		if (typeof(MktPage) !== "undefined") {
			cust_prefix_app = MktPage.savedState.custPrefix;
			//console.log(cust_prefix_app);
			cust_prefix_cookie = getCookie('custPrefix');
			//console.log(cust_prefix_cookie);
			if (cust_prefix_app !== undefined) {
				if (cust_prefix_app !== cust_prefix_cookie) {
					deleteCookie('custPrefix');
				}
				cust_prefix = cust_prefix_app;
	...
	function getPod() {
		if (typeof(MktPage) !== "undefined") {
			pod_app_full = MktPage.savedState.mainAppBaseUrl;
			pod_app_split = pod_app_full.split('//');
			pod_app_values = pod_app_split[1].split('.');
			pod_app = pod_app_values[0];
			pod_cookie = getCookie('userPod');
			if (pod_app !== undefined) {
				if (pod_app !== pod_cookie) {
					deleteCookie('userPod');
				}
				current_pod = pod_app;
	...
	function getCustPrefix(var_cust_prefix) {
		//console.log(var_cust_prefix);
		if (var_cust_prefix !== false) {
			window.postMessage({
				type : 'custPrefix',
				text : var_cust_prefix
			}, mkto_app_url);
			return true;
		}else{
			return false;
	...
	function getCurrentPod(var_current_pod) {
		//console.log(var_current_pod);
		if (var_current_pod != false) {
			window.postMessage({
				type : 'userPod',
				text : var_current_pod
			}, mkto_app_url);
			return true;
		}else{
			return false;
	...
	var custPrefixStatus = getCustPrefix(getPrefix());
	//console.log(custPrefixStatus);
	var currentPodStatus = getCurrentPod(getPod());
	//console.log(currentPodStatus);

	if (custPrefixStatus == true && currentPodStatus == true) {
		if (supported_prefixes.indexOf(cust_prefix) >= 0) {
			setCookie('custPrefix', cust_prefix, 365, 'marketo.com', true);
		}
		if (supported_pods.indexOf(current_pod) >= 0) {
			setCookie('userPod', current_pod, 365, 'marketo.com', true);
			//sendMktoLoginSuccess();
		}
	}else{
		console.log('Prefix Status = '+custPrefixStatus+' & Pod Status = '+currentPodStatus);
		console.log('Prefix = '+cust_prefix+' & Pod = '+current_pod);
	}
}
// Send Messages to Plugin
function sendMktoLoginSuccess() {
	window.postMessage({
		type : 'DemoMsg',
		text : 'login_success'
	}, mkto_app_url);
}
function sendDemoScriptComplete() {
	window.postMessage({
		type : 'DemoMsg',
		text : 'mkto_live_plugin_loaded'
	}, mkto_app_url);
}
//Set User Cookie
...
function getSmartCampaigns() {
	...
	var url = 'https://'+current_pod+'.marketo.com/explorer/getCampaignInspectorData?query=&zoneId=1&start=0&limit=30&pageSize=10000&compId=&xsrfId='+MktSecurity.getXsrfId();
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			var scResults = JSON.parse(xmlhttp.responseText);
			var i;
			console.log(scResults.data.campaigns.length);
			for ( i = 0; i < scResults.data.campaigns.length; i++) {
				var dItem = '#'+scResults.data.campaigns[i].campaign_id;
				//console.log(dItem);
				parms.push(dItem);
			}
			//console.log(parms);
...
function getEmails() {
	var parms = [];
	var xmlhttp = new XMLHttpRequest();
	var url = 'https://'+current_pod+'.marketo.com/email/showEmails?query=&zoneId=1&start=0&limit=30&pageSize=10000&compId=&xsrfId='+MktSecurity.getXsrfId();
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			console.log('Get Emails');
			var emResults = JSON.parse(xmlhttp.responseText);
			var i;
			console.log(emResults.data.length);
			for ( i = 0; i < emResults.data.length; i++) {
				var dItem = emResults.data[i].id;
				if (emResults.data[i].status == 'Approved with draft') {
					parms.push(dItem);	
				}				
			}
			console.log(parms);
...
function getLandingPages() {
	var parms = {};
	var xmlhttp = new XMLHttpRequest();
	var url = 'https://'+current_pod+'.marketo.com/landingPage/showPages?query=&zoneId=1&start=0&limit=30&pageSize=10000&compId=&xsrfId='+MktSecurity.getXsrfId();
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			console.log('Get Landing Pages');
			var lpResults = JSON.parse(xmlhttp.responseText);
			var i;
			console.log(lpResults.data.length);
			for ( i = 0; i < lpResults.data.length; i++) {
				if (lpResults.data[i].lpStatus == 2) {
					var dItem = 'dpageid_' + lpResults.data[i].id;
					parms[dItem] = dItem;	
				}
			}
			console.log(parms); 
...
function getForms() {
	var parms = [];
	var xmlhttp = new XMLHttpRequest();
	var url = 'https://'+current_pod+'.marketo.com/landingPage/showForms?query=&zoneId=1&start=0&limit=30&pageSize=100000&compId=&xsrfId='+MktSecurity.getXsrfId();
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			console.log('Get Forms');
			var fmResults = JSON.parse(xmlhttp.responseText);
			var i;
			console.log(fmResults.data.length);
			for ( i = 0; i < fmResults.data.length; i++) {
				var dItem = fmResults.data[i].id;
				if (fmResults.data[i].statusText == 'Approved with draft') {
					parms.push(dItem);	
				}				
			}
			console.log(parms);
...
function prevent_autosave_smartcampaigns(parms_array) {
		...
		if (Mkt.widgets.DataPanelLayout) {
			Mkt.widgets.DataPanelLayout.prototype.createManager = function() {
				...
				var currentSC,
				    scRegExp;
				for (var i = 0,
				    tot = parms_array.length; i < tot; i++) {
				    console.log(parms_array);
					currentSC = window.location.hash.slice(0, ((window.location.hash.length) - 2));
					console.log('Current Smart Campaign = ' + currentSC);
					console.log('Array Smart Campaign = ' + parms_array[i]);
					scRegExp = new RegExp(currentSC);
					console.log(scRegExp.test(parms_array[i]));
					if (scRegExp.test(parms_array[i]) == true) {
						console.log('Current Smart Campaign = ' + currentSC);
						console.log('Array Smart Campaign = ' + parms_array[i]);
	...
	Ext4.getStore('Email').load({
		...
		callback : function(records) {
			...
			records.forEach(function(record) {
				//console.log(record)
				var dItem = record.get('approvedAssetId');
				if (dItem !== null) {
					//console.log(dItem);
					parms.push(dItem);	
				}
			...
			console.log('Discarding Email Drafts');
			if (typeof(mktEmManager) !== 'undefined') {
				mktEmManager.discardDraft(parms);
				Ext.MessageBox.hide();
			}
		}
	});
...
/*
	debugger;
	MktSession.ajaxRequest('landingPage/showPages', {
		serializeParms: {
			zoneId: 1,
			start: 0,
			limit: 30,
			pageSize: 10000
		},onMySuccess: function(results, request) {						
			//Ext.each(results.data, function(results) {
				console.log(results);
				//if (results.lpStatus == 2) {
					//console.log(results);
				//}				
			//});
		} 
...
/*	
	...
	Ext4.getStore('LandingPage').load({
		...
		callback : function(records) {
			//debugger;			
			records.forEach(function(record) {
				if (record.get('id') !== record.get('principalAssetId')) {
					//console.log(record)
					var dItem = 'dpageid_' + record.get('principalAssetId');
					//console.log(dItem);
					parms[dItem] = dItem;
				}				
			});
...
function discard_fm_drafts() {
	...
	Ext4.getStore('Form').load({
		...
		callback : function(records) {
			//debugger;			
				records.forEach(function(record) {
				//console.log(record)
				var dItem = record.internalId;
				//console.log(dItem);
				parms.push(dItem);
				record.discard(function(success) {
					//console.log(success);
					if (success == false) {
						MktMessage._cleanupMessageWindow();
					}
					if (success == true) {
						record.updateNode();
					}
	...
	console.log('Discarding Form Drafts');
	console.log(parms);
}
//<---- This function discards social drafts
function discard_so_drafts() {
	var parms = [];
	...
	console.log('Discarding Social Drafts');
	console.log(parms);
	//TODO	
}
...
function update_calendar(var_cal_view) {
	...
	function getTiles() {
		...
		var allElements = document.querySelectorAll("[id*='homeTile']");
		for (var i = 0,
	...
	function getCalendarTile(matchingElements) {
		var targetElements = Array.prototype.slice.call(matchingElements);
		var cal_hash = '#CAL';	
		for (var j = 0,
		    o = targetElements.length; j < o; j++) {
			if (targetElements[j].hash.indexOf(cal_hash) == 0) {				
				targetEl = targetElements[j];
				break;
			}
		}
		if (document.getElementById(targetEl.id) !== null) {
			return targetEl;	
		}else{
			return false;
		}
	...
	function checkElements() {
		var targetTiles = getTiles();
		if (targetTiles.length < 0) {
			return false;
		}else if (targetTiles.length > 0) {
			var targetEl = getCalendarTile(targetTiles);
			var status;
			var i = 0;
			do {
	...
	var myVar = setInterval(function(){ 
		var element_state = checkElements();
		if (element_state == true) {
			clearInterval(myVar);
		}
	}, 1000);
}
	...
	document.getElementById('loginButton').onclick = function(e) {
		...
		if (type == 'live') {
			if (demoUser == true && demoType == 'Business') {
				...
				//--> Remove this line once the subscription is active
				e.preventDefault();
				document.getElementById('login-container').style.top = ('64px');
				currentError = setTimeout(getError, 3000);
				console.log(currentError);	
...
function cleanMarketingActivities() {
...
function addSuperBallEvents(current_pod, cal_view) {
...
var get_url = current_url;

var supported_prefix_index = supported_prefixes.indexOf(cust_prefix);

if (supported_prefixes.indexOf(cust_prefix) >= 0 && isAdminUser == false) {
	console.log(supported_prefixes.indexOf(cust_prefix));
	console.log(cust_prefix);
	console.log(current_pod);
	var mkto_live_plugin_state = true;
	if (supported_prefixes[supported_prefix_index] == cust_prefix) {
		console.log('Demo Account Supported');
		addSuperBallEvents(current_pod, cal_view[supported_prefix_index]);
		if (get_url == 'https://'+current_pod+'.marketo.com/#MM0A1') {
			cleanMarketingActivities();			
		}
		if (domain_s_1.indexOf('https://'+current_pod+'.marketo.com/m') !== 0 || location.hash !== '#CAL' || location.hash !== '#CALPR') {
			window.Mkt3 && Mkt3.onLaunch(function() {
				var scArray = getSmartCampaigns();
				var emArray = getEmails();
				var lpArray = getLandingPages();
				var fmArray = getForms();
				/*
				protectMarketingActivities(
					cal_view[supported_prefix_index],
					scArray, 
					emArray,
					lpArray,
					fmArray[supported_prefix_index], 
					soArray[supported_prefix_index], 
					ceArray[supported_prefix_index]
					);
				*/
			});
		}
		if (domain_s_1.indexOf('https://'+current_pod+'.marketo.com/m') == 0) {
			var buttons = Ext4.ComponentQuery.query('button[action=approveAndClose]');
			buttons.forEach(function(button) {
				button.disable();
			});
		}
		if (domain_s_1 == 'https://'+current_pod+'.marketo.com/' || domain_s_1 == 'https://'+current_pod+'.marketo.com/?preview=true&approved=true/') {
		...
		if (get_url == 'https://'+current_pod+'.marketo.com/#MM0A1') {
			...
			console.log('Demo Account NOT Supported');
		}

}
else if (supported_prefixes.indexOf(cust_prefix) >= 0 && isAdminUser == true) {
...
if (cust_prefix == 'mktodemoaccount106' && isAdminUser == false) {
	...
	addSuperBallEvents(current_pod, cal_view);

	if (get_url == 'https://app-sjp.marketo.com/#MM0A1') {
		cleanMarketingActivities();
	}
	if (domain_s_1.indexOf('https://app-sjp.marketo.com/m') !== 0 || location.hash !== '#CAL' || location.hash !== '#CALPR') {
		window.Mkt3 && Mkt3.onLaunch(function() {
...
/*
	...
	if (domain_s_1.indexOf('https://app-ab07.marketo.com/m') !== 0 || location.hash !== '#CAL' || location.hash !== '#CALPR') {
		window.Mkt3 && Mkt3.onLaunch(function() {
...
/*
	...
	if (domain_s_1.indexOf('https://app-ab08.marketo.com/m') !== 0 || location.hash !== '#CAL' || location.hash !== '#CALPR') {
		window.Mkt3 && Mkt3.onLaunch(function() {
