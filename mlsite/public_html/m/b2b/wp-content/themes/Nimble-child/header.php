<?php $requri = $_SERVER['HTTP_REFERER'];
$matchurl = '/^(https?:\/\/marketolive.com\/m\/(.*))/';
$url = '//' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
//redirect to the poll use case landing page
if (false !== strpos($url, 'poll')) {
	header('Location: http://pages.marketolive.com/Poll.html');
	exit ;
}

if (!isset($_COOKIE['_mkto_trk']))
{
    header('Location: //marketolive.com/m/');
    exit;
}
if (false !== strpos($url, 'agile')){
	//echo $requri;
	if (!preg_match($matchurl, $requri))
	{
		header('Location: //marketolive.com/m/');
		exit;
	}
...
<!-- Google Tag Manager -->
<noscript><iframe src="//www.googletagmanager.com/ns.html?id=GTM-K4L446"
...
<script>
	...
	})(window, document, 'script', 'dataLayer', 'GTM-K4L446'); 
</script>
<!-- End Google Tag Manager -->
	...
	<header id="top-header" class="container">
		<div class="container">
			<div id="top-area" class="clearfix">
				...
				<a href="//marketolive.com/m"><img src="<?php echo esc_url($logo); ?>" alt="<?php echo esc_attr(get_bloginfo('name')); ?>" id="logo"/></a>
 ...
 ?>><a href="<?php echo esc_url(home_url('/')); ?>"><?php esc_html_e('Home','Nimble') ?></a></li>
								<?php } ?>
 ...
 ?>
		<!--</div>--> <!-- end .container -->
		<?php
			if ('on' == et_get_option('nimble_featured', 'on') && is_home())
				get_template_part('includes/featured', 'home');
 ?>
