  ...
  $.mbExtruder= {
    ...
    buildMbExtruder: function(options){
      return this.each (function (){
        ...
        //flapLabel.text(txt);
        if(isVertical){
          //if the flap is vertical this code flips the text (note: our flap is vertical)
        ...
        if (isVertical){
          c.css({ "margin-top":"35px", height:"90%"});
          if(this.options.top=="auto") {
            flap.css({top:181+(this.options.position=="left"?document.extruder.left:document.extruder.right)});
    ...
    setMbExtruderContent: function(options){
      ...
      var container=$("<div>").addClass("containerside");
      ...
      $.jsonp({
        ...
        success: function(data, textStatus, xOptions){
        	//where.find(".container").append('<div id="accordion" style="height:100%"> <h3>Personal Information</h3> <div style="color:white">First content panel</div> <h3>Website Activity</h3> <div>Second content panel</div> <h3>Email Activity</h3> <div>First content panel</div> <h3>Interesting Moments</h3> <div>First content panel</div> <h3>Your Score</h3> <div>First content panel</div> </div>');
        	$('#loading-image').remove();
        	sidetabdata = data.html;
        	where.find(".containerside").append(sidetabdata);
        },
        error: function (xhr, ajaxOptions, thrownError) {
			$('#loading-image').empty();
			$('#loading-image').css({"background-image":"none","width":"85px"});
			$('#loading-image').html('<a id="reloadforreal" href=#><div id="reloadforrealbutton">Load Activity</div></a>');			
			$('#reloadforreal').click(function(){
				$('#reloadforrealbutton').empty();
				$('#reloadforrealbutton').text("Retrieving...");
				console.log(url);
				console.log(data);
				$.jsonp({
		        type: "GET",
    ...
    closeMbExtruder:function(){
    	var visibility = $('.reveal-modal').css('visibility');
...
//Setting the Z-Index of the SIDETAB
//Setting the Z-Index of the SIDETAB
//Setting the Z-Index of the SIDETAB
//Setting the Z-Index of the SIDETAB
